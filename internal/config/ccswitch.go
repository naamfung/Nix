...
// LoadCCSwitchMCP reads MCP servers enabled for Inx from cc-switch and maps
// them to Inx plugin entries. Newer cc-switch stores servers in SQLite;
// older installs kept them in config.json(.migrated/.bak), so we support both.
//
// CC Switch v16+ stores dedicated enabled_inx / apps.inx flags.
// Treat those as authoritative when present, and fall back to Codex only for
// pre-v16 SQLite schemas or legacy JSON entries without Inx enablement.
...
func loadCCSwitchMCPFromRoot(root string) ([]PluginEntry, error) {
	...
	return nil, fmt.Errorf("cc-switch import: no Inx-enabled MCP servers found in %s", root)
}
...
// ImportCCSwitchMCP upserts cc-switch's Inx-enabled MCP servers into the
// user-global Inx config and saves it.
...
func loadCCSwitchMCPDB(path string) ([]PluginEntry, error) {
	...
	hasInx, err := ccSwitchDBHasInxColumn(sqlite, path)
	if err != nil {
		return nil, err
	}
	if hasInx {
		query = `SELECT id, name, server_config FROM mcp_servers WHERE enabled_inx = 1 ORDER BY name, id`
	}
...
func ccSwitchDBHasInxColumn(sqlite, path string) (bool, error) {
	const query = `SELECT COUNT(*) FROM pragma_table_info('mcp_servers') WHERE name = 'enabled_inx'`
	...
	case "1":
		return true, nil
	default:
		return false, fmt.Errorf("cc-switch import: inspect %s: unexpected enabled_inx column count %q", path, strings.TrimSpace(string(out)))
	}
}
