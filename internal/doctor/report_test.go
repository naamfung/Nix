...
func TestCollectReportRedactsSecrets(t *testing.T) {
	t.Setenv("INX_TEST_SECRET", "sk-live-secret")
...
func TestCollectReportDoesNotRequireAPIKey(t *testing.T) {
	t.Setenv("INX_HOME", filepath.Join(t.TempDir(), "inx"))
	...
	if !strings.Contains(text, "inx 1.2.3 doctor") {
...
func TestRenderTextSurfacesWarningsUpTop(t *testing.T) {
	text := RenderText(Report{Warnings: []string{"config inx.toml: parse boom"}})
