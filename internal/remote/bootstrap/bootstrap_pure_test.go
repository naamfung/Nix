...
func TestLaunchCommandQuotesHostilePaths(t *testing.T) {
	paths := StatePaths{
		Dir:       "/home/dev/.inx/remote",
		TokenFile: "/home/dev/.inx/remote/serve-x.token",
		PortFile:  "/home/dev/.inx/remote/serve-x.port",
		PidFile:   "/home/dev/.inx/remote/serve-x.pid",
		LogFile:   "/home/dev/.inx/remote/serve-x.log",
	}
	hostile := "/tmp/'; rm -rf ~; echo '"
	cmd := LaunchCommand("/usr/bin/inx", hostile, paths)
...
func TestStopAndServeAliveCommands(t *testing.T) {
	...
	// Must check liveness AND that the process is a inx serve (guards PID
	// reuse), not just kill -0.
	for _, want := range []string{"kill -0 99", "ps -p 99", "*inx*serve*", paths.TokenFile, paths.PortFile} {
...
func TestLaunchCommandDetachAndLogHardening(t *testing.T) {
	cmd := LaunchCommand("/usr/bin/inx", "/ws", StatePaths{
...
func TestLocateCommandProbesPortFileFlag(t *testing.T) {
	cmd := LocateCommand("/home/x/.inx/remote/bin/inx")
