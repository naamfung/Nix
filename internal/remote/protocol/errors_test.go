...
func TestFrozenErrorTableHasExactly51ControlledEntries(t *testing.T) {
	...
	seen := map[InxErrorCode]bool{}
	gotCodes := make([]string, 0, len(contracts))
	for _, contract := range contracts {
		gotCodes = append(gotCodes, string(contract.InxCode))
		if seen[contract.InxCode] {
			t.Fatalf("duplicate error %s", contract.InxCode)
		}
		seen[contract.InxCode] = true
		if contract.JSONRPCCode != DomainErrorCode || strings.TrimSpace(contract.Message) == "" {
			t.Fatalf("invalid contract %+v", contract)
		}
		options := ErrorOptions{}
		if contract.InxCode == ErrHostBusy {
			retry := int64(100)
			options.RetryAfterMs = &retry
		}
		if contract.InxCode == ErrRewindPartial {
		...
		remoteErr, err := NewRemoteError(contract.InxCode, options)
		if err != nil {
			t.Fatalf("construct %s: %v", contract.InxCode, err)
		}
		if remoteErr.Message != contract.Message || remoteErr.RPCError().Code != DomainErrorCode {
			t.Fatalf("constructed error drift for %s", contract.InxCode)
		}
	}
...
PATH_NOT_FOUND NOT_FILE GIT_UNAVAILABLE GIT_OBJECT_NOT_FOUND QUERY_FAILED CAPABILITY_UNAVAILABLE`)
	...
	if !reflect.DeepEqual(gotCodes, wantCodes) {
		t.Fatalf("Inx error code golden drift\n got: %v\nwant: %v", gotCodes, wantCodes)
	}
}
...
func TestRemoteErrorConditionalFields(t *testing.T) {
	...
	data.SuggestedCommand = "inx remote start; rm -rf /"
