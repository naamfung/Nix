...
import (
	...
	// Blank import registers the provider kind the same way cmd/inx's main
...
// cache-first wiring: a project INX.md is discovered at boot and folded
...
`)
	writeFile(t, dir, "INX.md", "Project rule: always run go vet before committing.")
	...
	if !strings.Contains(sys, "always run go vet before committing") {
		t.Fatalf("project INX.md not folded into system message:\n%s", sys)
	}
	...
	if mem := ctrl.Memory(); mem == nil || len(mem.Docs) == 0 {
		t.Fatal("controller memory set is empty after discovering INX.md")
	}
}
...
func TestBuildRunsCleanupPendingDespiteSafeModeEnv(t *testing.T) {
	// v1.20+: INX_SAFE_MODE no longer skips cleanup reconciliation.
	isolateConfigHome(t)
	dir := robustTempDir(t)
	t.Chdir(dir)
	...
	defer ctrl.Close()
	if !called {
		t.Fatal("cleanup-pending reconciler must still run when INX_SAFE_MODE is set")
	}
}
...
model = "x"
`)
	writeFile(t, dir, ".inx/skills/wskill.md",
		"---\ndescription: writer skill\nrunAs: subagent\nallowed-tools: bash, read_file, write_file\n---\nwriter body")
	writeFile(t, dir, ".inx/skills/roskill.md",
...
// `inx run`: a controller built via Build with NO SetSessionPath (exactly
...
func TestRecoveryHeadlessModeUsesExplicitFrontendCapability(t *testing.T) {
	...
	if !recoveryHeadlessMode(Options{HeadlessApprovalMode: control.ToolApprovalAuto}) {
		t.Fatal("inx run Auto mode must fail closed instead of waiting for a card")
	}
...
api_key_env = "INX_TEST_KEY_UNSET"
`)
	writeFile(t, dir, ".inx/skills/projskill.md", "---\ndescription: a project skill\n---\nplaybook")
...
func TestBuildDiscoversSkillsDespiteSafeModeEnv(t *testing.T) {
	// v1.20+: skill discovery is not gated by INX_SAFE_MODE.
	dir := robustTempDir(t)
	home := robustTempDir(t)
	t.Setenv("HOME", home)
	t.Setenv("XDG_CONFIG_HOME", filepath.Join(home, ".config"))
	t.Setenv("INX_SAFE_MODE", "1")
	t.Chdir(dir)
	writeFile(t, dir, ".inx/skills/project-skill.md", "---\ndescription: project skill\n---\nplaybook")
	writeFile(t, home, ".inx/skills/global-skill.md", "---\ndescription: global skill\n---\nplaybook")
	...
	if skills := ctrl.AllSkills(); len(skills) == 0 {
		t.Fatal("skills must still be discovered when INX_SAFE_MODE is set")
	}
}
...
api_key_env = "INX_TEST_KEY_UNSET"
`)
	pluginRoot := filepath.Join(inxHome, "plugins", "superpowers")
...
model = "planner-model"`
		...
		writeFile(t, dir, "inx.toml", fmt.Sprintf(`
...
model = "x"
`)
			writeFile(t, dir, ".inx/commands/check.md", "---\ndescription: inspect the project\n---\ninspect $ARGUMENTS")
...
model = "x"
`)
	writeFile(t, dir, ".inx/skills/readonlydig/SKILL.md", `---
...
func TestBuildOmitsExcludedSkillRootsFromPromptAndRuntimeList(t *testing.T) {
	...
	writeFile(t, home, ".inx/skills/keep.md", "---\ndescription: keep\n---\nplaybook")
...
`)
	...
	// user-global INX.md in the real config dir could append; the test
...
func TestRememberPermissionRuleUsesWorkspaceRoot(t *testing.T) {
	...
	t.Chdir(cwd)
	writeFile(t, cwd, "inx.toml", `
[permissions]
allow = ["Bash(cwd*)"]
`)
	writeFile(t, workspace, "inx.toml", `
[permissions]
...
`)
	...
	cwdCfg := config.LoadForEdit(filepath.Join(cwd, "inx.toml"))
	...
	workspaceCfg := config.LoadForEdit(filepath.Join(workspace, "inx.toml"))
...
`)
	...
	path := filepath.Join(workspace, "inx.toml")
...
func TestRememberPermissionRuleIgnoresTOMLExampleInMultilineSystemPrompt(t *testing.T) {
	workspace := robustTempDir(t)
	writeFile(t, workspace, "inx.toml", `[agent]
system_prompt = """
Example only:
[permissions]
allow = ["Bash(example)"]
...
func TestRememberPermissionRuleSerializesConcurrentWriters(t *testing.T) {
	workspace := robustTempDir(t)
	writeFile(t, workspace, "inx.toml", "[permissions]\nallow = []\n")
	...
	got := config.LoadForEdit(filepath.Join(workspace, "inx.toml"))
	for i := 0; i < writers; i++ {
...
func TestRememberPermissionRuleSerializesCrossProcessWriters(t *testing.T) {
	...
	for worker := 0; worker < workers; worker++ {
		...
		cmd.Env = append(os.Environ(),
			"INX_PERMISSION_HELPER=1",
			"INX_PERMISSION_WORKSPACE="+workspace,
			"INX_PERMISSION_READY_DIR="+readyDir,
			"INX_PERMISSION_START="+startPath,
			fmt.Sprintf("INX_PERMISSION_WORKER=%d", worker),
			fmt.Sprintf("INX_PERMISSION_RULES=%d", rulesPerWorker),
		)
...
func TestRememberPermissionRuleProcessHelper(t *testing.T) {
	if os.Getenv("INX_PERMISSION_HELPER") != "1" {
		return
	}
	workspace := os.Getenv("INX_PERMISSION_WORKSPACE")
	readyDir := os.Getenv("INX_PERMISSION_READY_DIR")
	startPath := os.Getenv("INX_PERMISSION_START")
	t.Setenv("INX_CACHE_HOME", readyDir)
	worker, err := strconv.Atoi(os.Getenv("INX_PERMISSION_WORKER"))
	if err != nil {
		t.Fatal(err)
	}
	rules, err := strconv.Atoi(os.Getenv("INX_PERMISSION_RULES"))
	if err != nil {
		t.Fatal(err)
	}
...
`)
	...
	if !res.Saved || res.Path != filepath.Join(workspace, "inx.toml") {
...
`)
	...
	if _, err := os.Stat(filepath.Join(cwd, "inx.toml")); !os.IsNotExist(err) {
...
`)
	...
	cfg := config.LoadForEdit(filepath.Join(workspace, "inx.toml"))
...
`)
	...
	const literal = "Bash=git status $(touch /tmp/inx-dynamic-approval)"
...
// ~/.inx/config.json with no v1+ config present must be imported during
...
func TestBuildMigratesLegacyConfigEndToEnd(t *testing.T) {
	...
	// migrated plugins.
	writeFile(t, proj, "inx.toml", "")
	writeFile(t, filepath.Join(home, ".inx"), "config.json",
		`{"apiKey":"sk-e2e","lang":"zh","mcpServers":{"fs":{"command":"npx","args":["-y","server-fs"]}}}`)
	writeFile(t, filepath.Join(home, ".inx", "sessions"), "chat-1.events.jsonl",
...
func TestBuildMigratesDeprecatedAgentStepLimitsWithOneNotice(t *testing.T) {
	...
	t.Setenv("INX_HOME", filepath.Join(home, "inx-home"))
	project := robustTempDir(t)
	configPath := filepath.Join(project, "inx.toml")
	writeFile(t, project, "inx.toml", `
default_model = "test-model"
...
func TestBuildSkipsLegacySessionMigrationWhenIsolated(t *testing.T) {
	...
	inxHome := filepath.Join(home, "rx-home")
	t.Setenv("HOME", home)
	t.Setenv("USERPROFILE", home)
	...
	proj := robustTempDir(t)
	writeFile(t, proj, "inx.toml", "[codegraph]\nenabled = false\n")

	legacyRoot := filepath.Join(xdg, "inx")
	...
	if _, err := os.Stat(filepath.Join(config.SessionDir(), "xdg-flat.jsonl")); !os.IsNotExist(err) {
		t.Fatal("legacy XDG flat session was imported but must not be when INX_HOME is set")
	}
	projectPath := filepath.Join(config.MemoryUserDir(), "projects", slug, "sessions", "project-chat.jsonl")
	if _, err := os.Stat(projectPath); !os.IsNotExist(err) {
		t.Fatal("legacy project session was imported but must not be when INX_HOME is set")
	}
}
...
[[plugins]]
name = "legacy-eager"
command = "inx-missing-legacy-eager-mcp"
tier = "eager"
`)
	...
	raw, err := os.ReadFile(filepath.Join(dir, "inx.toml"))
	if err != nil {
		t.Fatal(err)
	}
...
[[plugins]]
name = "legacy-lazy"
command = "inx-missing-legacy-lazy-mcp"
tier = "lazy"
`)
...
func TestRuntimeForbidReadRootsFiltersUnconfiguredStoredCredential(t *testing.T) {
	home := isolateConfigHome(t)
	t.Setenv("INX_HOME", filepath.Join(home, "inx-home"))
	const staleKey = "INX_TEST_UNCONFIGURED_STORED_CREDENTIAL"
...
[[plugins]]
name = "slowserver"
command = "inx-missing-slow-mcp-binary"
tier = "eager"
`)
...
// v1.20+ no longer strips tools when INX_SAFE_MODE is set.
func TestBuildKeepsSourceConnectorAndSkillToolsDespiteSafeModeEnv(t *testing.T) {
	...
	for _, want := range []string{"install_source", "run_skill", "slash_command"} {
		if !names[want] {
			t.Fatalf("expected %s when INX_SAFE_MODE is set", want)
		}
	}
}
