...
func InstalledNames(inxHome string) ([]string, error) {
...
func InstalledListText(inxHome string) (string, error) {
	st, err := LoadState(inxHome)
	if err != nil {
		return "", err
	}
	if len(st.Plugins) == 0 {
		return "plugins: none installed\ninstall: inx plugin install <source> --yes, or use Settings -> Plugins", nil
	}
	...
	for _, p := range st.Plugins {
		...
		counts := pluginCapabilityText(inxHome, p)
		if summary != "" && counts != "" {
...
func InstalledShowText(inxHome, name string) (string, error) {
	p, ok, err := FindInstalled(inxHome, name)
	if err != nil {
		return "", err
	}
	if !ok {
	...
	root := ResolveRoot(inxHome, p.Root)
...
func FindInstalled(inxHome, name string) (InstalledPlugin, bool, error) {
	st, err := LoadState(inxHome)
	if err != nil {
...
func pluginCapabilityText(inxHome string, p InstalledPlugin) string {
	root := ResolveRoot(inxHome, p.Root)
