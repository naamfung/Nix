...
func TestRemoteSSHTransportExactArgvAndProtocolPurity(t *testing.T) {
	...
	wantArgs := []string{
		...
		"inx", "remote", "attach-workspace", "--stdio",
	}
...
func TestRemoteSSHTransportConfiguredAliasPreservesExplicitOverrides(t *testing.T) {
	...
	wantTail := []string{
		...
		"--", "gpu.corp", "inx", "remote", "attach-workspace", "--stdio",
	}
...
func TestRemoteSSHTransportUsesProvisionedWorkbenchBinary(t *testing.T) {
	...
	var gotArgs []string
	factory := &RemoteSSHTransportFactory{
		RemoteBinary: "/home/dev user's/.inx/remote/workbench/abc/inx",
...
func TestRemoteSSHTransportClassifiesControlledFailures(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.mode, func(t *testing.T) {
			...
			for _, raw := range []string{"Permission denied", "REMOTE HOST IDENTIFICATION", "inx: command not found"} {
...
func TestRemoteSSHTransportCLIMissingAfterProtocolIsTransportLost(t *testing.T) {
	failure := classifyRemoteSSHExit([]byte("inx: command not found"), true, nil)
...
func TestRemoteSSHHostTransportFactoryBindsDirectConnection(t *testing.T) {
	...
	t.Cleanup(func() { _ = transport.Close() })
	wantTail := []string{"-l", "developer", "-p", "2200", "--", "2001:db8::20", "inx", "remote", "attach-workspace", "--stdio"}
	if len(gotArgs) < len(wantTail) || !reflect.DeepEqual(gotArgs[len(gotArgs)-len(wantTail):], wantTail) {
...
func TestRemoteSSHTransportAskPassEnvironmentIsControlled(t *testing.T) {
	...
	helper := filepath.Join(t.TempDir(), "inx-desktop")
...
func TestRemoteSSHEnvironmentIsAllowlisted(t *testing.T) {
	env := parseEnvironmentMap(sanitizeRemoteSSHEnvironment([]string{
		...
		"INX_REMOTE_ASKPASS_KEY=must-not-cross", "UNRELATED_APP_STATE=value",
	}))
...
func TestRemoteSSHFakeProcess(t *testing.T) {
	...
	case "protocol":
		...
		os.Exit(0)
	case "huge-cli":
		_, _ = io.WriteString(os.Stderr, "inx: command not found\n")
	...
	case "cli":
		_, _ = io.WriteString(os.Stderr, "sh: inx: command not found\n")
	case "generic":
