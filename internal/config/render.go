...
// RenderTOML renders the config as annotated TOML in the `inx setup` house style:
...
// inx.toml stays focused on project behavior and intentionally excludes
...
func RenderTOMLForScope(c *Config, scope RenderScope) string {
	...
	b.WriteString("# Inx configuration.\n")
	fmt.Fprintf(&b, "# Resolution order: flag > ./inx.toml > %s > built-in defaults.\n", userConfigDisplayPath())
	b.WriteString("# Fields marked user/global only are not overridden by ./inx.toml.\n")
	b.WriteString("# Secrets are named via api_key_env and stored in Inx's global .env; never put keys here.\n\n")

	fmt.Fprintf(&b, "config_version = %d   # schema marker for diagnostics; old versions may ignore it\n", configVersion(c))
	fmt.Fprintf(&b, "default_model = %q\n", c.DefaultModel)
	if c.Language != "" {
		fmt.Fprintf(&b, "language      = %q   # ui/model language; empty = auto-detect from $LANG / $INX_LANG\n", c.Language)
	} else {
		b.WriteString("# language      = \"zh\"   # ui/model language; empty = auto-detect from $LANG / $INX_LANG\n")
	}
	if scope != RenderScopeProject {
		fmt.Fprintf(&b, "credentials_store = %q   # legacy compatibility; provider keys are saved in Inx's global .env\n", normalizeCredentialsStore(c.CredentialsStore))
	}
	b.WriteString("\n")

	if shouldRenderUI(c, defaults, scope) {
		b.WriteString("[ui]\n")
		fmt.Fprintf(&b, "theme = %q   # auto|dark|light; CLI colors only; INX_THEME can override per run\n", c.UITheme())
		if style := c.UIThemeStyle(); style != "" {
			fmt.Fprintf(&b, "theme_style = %q   # CLI accent palette; INX_THEME_STYLE can override per run\n", style)
		} else {
	...
	if scope != RenderScopeProject {
		...
		// configs. It is the only desktop field written to inx.toml: local
	...
	if shouldRenderNetwork(c, defaults, scope) {
		...
		if c.Network.Proxy.Password != "" {
			...
			b.WriteString("# password = \"${INX_PROXY_PASSWORD}\"   # optional; supports ${VAR} expansion\n")
		}
		b.WriteString("\n")
	}
	...
	if shouldRenderBot(c, defaults, scope) {
		...
		if strings.TrimSpace(c.Bot.Control.TokenEnv) != "" {
			...
			b.WriteString("# token_env = \"INX_BOT_CONTROL_TOKEN\"\n")
		}
	...
	if scope != RenderScopeProject {
		b.WriteString("[secrets]   # credential protection; user/global only, ./inx.toml cannot override\n")
	...
	if scope != RenderScopeProject && (c.Remote.ImportSSHConfig || len(c.Remote.Hosts) > 0) {
		b.WriteString("[remote]   # SSH remote hosts; user/global only, ./inx.toml cannot override\n")
		if c.Remote.ImportSSHConfig {
			b.WriteString("import_ssh_config = true   # surface ~/.ssh/config aliases in `inx remote import`\n")
		}
		for _, h := range c.Remote.Hosts {
			...
			if h.IdentityFile != "" {
				fmt.Fprintf(&b, "identity_file = %q   # key file path; Inx never stores key material\n", h.IdentityFile)
			}
			if h.PassphraseEnv != "" {
				fmt.Fprintf(&b, "passphrase_env = %q   # env var name; value lives in Inx's global .env\n", h.PassphraseEnv)
			}
			if h.PasswordEnv != "" {
				fmt.Fprintf(&b, "password_env = %q   # env var name; value lives in Inx's global .env\n", h.PasswordEnv)
			}
			if h.ProxyJump != "" {
	...
	if len(plugins) == 0 {
		...
		b.WriteString("# command = \"inx-plugin-example\"\n")
...
// active TOML array; an empty one renders as a commented example so `inx setup`
