// Package installlayout implements the Inx v1.20+ versioned install layout:
// InstallRoot/{current.json, inx-launcher, versions/<version>/...}.
//
...
func UpdateHelperBinaryName() string {
	if runtime.GOOS == "windows" {
		return "inx-update-helper.exe"
	}
	return "inx-update-helper"
}
...
func LauncherBinaryName() string {
	if runtime.GOOS == "windows" {
		return "inx-launcher.exe"
	}
	return "inx-launcher"
}

// PortableAliasName is the Windows portable entry (Inx.exe).
