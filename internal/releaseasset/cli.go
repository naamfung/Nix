// Package releaseasset downloads and verifies immutable Inx CLI release
...
func downloadCLIFromBase(ctx context.Context, client *http.Client, base, version, goos, goarch string, official bool) ([]byte, error) {
	...
	assetName := fmt.Sprintf("inx-%s-%s.tar.gz", goos, goarch)
...
func fetchBounded(ctx context.Context, client *http.Client, rawURL string, limit int64) ([]byte, error) {
	...
	req.Header.Set("User-Agent", "inx-remote-bootstrap")
...
func extractCLI(archive []byte) ([]byte, error) {
	...
	for {
		...
		if path.Base(path.Clean(header.Name)) != "inx" {
			continue
		}
		if header.Typeflag != tar.TypeReg || header.Size <= 0 || header.Size > maxExtractedCLIBytes {
			return nil, errors.New("inx archive entry is not a bounded regular file")
		}
		if binary != nil {
			return nil, errors.New("inx archive contains duplicate binaries")
		}
		...
		if int64(len(binary)) != header.Size {
			return nil, errors.New("inx archive entry size mismatch")
		}
	}
	if len(binary) == 0 {
		return nil, errors.New("inx binary not found in archive")
	}
	return binary, nil
}
