...
// loadDotEnv loads Inx's global .env for provider credentials. The
...
// expansion, then loads Inx's global .env for provider credentials.
...
// other and project files cannot redirect Inx's own config/credential
// paths.
...
func isProjectDotEnvControlKey(key string) bool {
	...
	if strings.HasPrefix(upper, "INX_") {
		return true
	}
	switch upper {
