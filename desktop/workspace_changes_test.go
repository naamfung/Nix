...
func TestWorkspaceGitBranchForMetaDoesNotBlockOnColdProbe(t *testing.T) {
	...
	start := time.Now()
	if got := workspaceGitBranchForMeta("/tmp/inx-cold-probe"); got != "" {
	...
	releaseProbe()
	eventuallyBranchForMeta(t, "/tmp/inx-cold-probe", "feature/async")
}

func TestWorkspaceGitBranchForMetaReturnsStaleDuringRefresh(t *testing.T) {
	resetWorkspaceGitBranchMetaCacheForTest(t)
	workspaceGitBranchCache.Lock()
	workspaceGitBranchCache.entries[filepath.Clean("/tmp/inx-stale-probe")] = workspaceGitBranchCacheEntry{
	...
	start := time.Now()
	if got := workspaceGitBranchForMeta("/tmp/inx-stale-probe"); got != "feature/stale" {
	...
	releaseProbe()
	eventuallyBranchForMeta(t, "/tmp/inx-stale-probe", "feature/fresh")
}
