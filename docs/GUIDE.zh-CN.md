...
优先级：**flag > `./inx.toml` > 用户配置文件 > 内置默认值**。从
**Inx v1.8.1** 开始，用户配置位于 macOS/Linux 的
`~/.inx/config.toml`，Windows 为 `%AppData%\inx\config.toml`；迁移和相关数据路径见
[配置路径](./CONFIG_PATHS.zh-CN.md)。标注为“仅用户/全局”的字段（包括 agent 轮数上限）不会被 `./inx.toml` 覆盖。
Provider 通过 `api_key_env` 命名密钥，真实密钥值保存在 CLI 与桌面端共用的
Inx 全局 `<Inx home>/.env`。项目 `.env`、home `.env`、继承的 shell 环境变量、旧 credentials 和系统 keyring 都不再作为 provider key 的运行时 fallback；旧凭据只作为迁移来源读取。项目 `.env` 仍会作为当前 workspace 范围内的 MCP/plugin 非 provider `${VAR}` 展开来源，但不会导入 provider key 或 Inx 控制变量。全局 `config.toml` 和 `.env` 的完整结构见
[配置路径](./CONFIG_PATHS.zh-CN.md)。
...
插件包可通过 `hooks/session-start-codex` 或插件根目录 `CLAUDE.md` 提供该启动上下文；Claude 风格 `.claude/settings.json` command hooks 也会按同名事件映射到 Inx hooks。

```toml
default_model = "deepseek-flash"   # 执行器；设 [agent].planner_model 可加规划器
# language    = "zh"               # 界面语言；为空则按 $LANG / $INX_LANG 自动检测
...
# password_hash = ""           # 用 inx serve --hash-password --password '...' 生成
...
多数日常设置应写在 `config.toml` 或前文提到的 Inx 全局 `.env` 中。下面这些变量是进程级高级开关；
需要在启动 Inx 之前设置。项目 `.env` 不是 Inx 控制变量的运行时来源。
...
Inx 绝不会上传 prompt、回答、reasoning、工具名/参数/输出、路径、仓库/分支、
session ID、精确 token/费用、Provider/model 名称、base URL 或环境变量。

### CLI 崩溃报告

当未处理的 Go panic 到达 CLI 入口调用栈时，Inx 会把脱敏报告保存在
`<Inx home>/cli-crash-reports`。最多保留 10 份，文件权限仅限当前用户读取。
...
```bash
inx report                 # 预览最新报告；TTY 中询问后才发送
inx report list            # 列出本地报告
inx report show [ID]       # 仅预览，不发送
inx report send [ID]       # 明确发送；成功后才删除本地副本
inx report delete [ID]     # 不发送，直接删除
```

通过 pipe 或重定向运行 `inx report` 时只会预览，不会询问或发送。CLI telemetry
设置不会自动发送或自动删除这些
...
`inx serve` 会用同一个本地 Inx 引擎启动浏览器 UI。适合不安装桌面端但想用可视化界面、
...
`inx acp` 把 Inx 作为 ACP v1 stdio agent 提供给编辑器和其他 host 客户端。
...
远程模块让 Inx 在远端主机上运行,并通过你自己的 SSH 连接访问它 —— 即 VS Code
Remote-SSH 式的体验。它在远端主机上引导一个常驻的 headless `inx serve`,把本地一个
...
`inx.toml` 无法注入或覆盖远程主机 —— 克隆的仓库永远无法左右 Inx 向何处发起 SSH
连接。凭据沿用 provider 惯例:主机只记录环境变量名(`passphrase_env`、`password_env`),其值
存放在 Inx 全局 `.env` 中;私钥内容本身从不存储 —— `identity_file` 只是路径。
...
inx remote import --all              # 导入别名；连接时通过 ssh -G 解析 Include/Match 等规则
inx remote test gpu-box              # 拨号 + 认证 + 主机密钥确认
inx remote connect gpu-box --open    # 引导 serve、建隧道、打开 URL
...
主机密钥会对照你的 OpenSSH `~/.ssh/known_hosts`(只读)以及 Inx 托管的
`~/.inx/remote/known_hosts` 校验。首次见到的密钥会提示 TOFU 确认并记入托管文件;与已记录
密钥冲突的密钥会硬失败并指明出错的行,绝不自动接受。

远端侧状态位于远端主机的 `~/.inx/remote/`:`serve-<工作区 slug>.json`(pid、绑定的回环
...
Remote SSH 的独立 Inx 原生窗口。主窗口持有 SSH 隧道；远程窗口是隔离的轻量外壳，不会恢复
或抢占本地对话会话。
...
可选预设，两者不会互相替换。Inx 还可以预填以下可编辑的自定义 provider：
...
预设路径通常只需要填写服务商 API Key：真实 key 会写入 Inx home `.env`，
...
上下文窗口以及官方 `low`/`high`/`max` 推理强度（默认 `max`）。对官方 K3 端点，Inx
...
**API 地址** 填写服务端点。默认模式下，Inx 会预览并把聊天请求发送到：
...
开启 **完整 URL**。开启后 Inx 会直接使用该地址，不再追加 `/chat/completions`。
输入框下方的预览就是最终请求地址。
...
| 字段 | 作用 | 什么时候改 |
| --- | --- | --- |
| `api_key_env` | 该 provider 使用的 API key 环境变量名。桌面端保存的真实 key 会写入 Inx home `.env` 的同名变量；TOML 配置里只保存变量名。 | 多个 provider 需要不同 key 时改名；服务不需要 API key 时可以留空。 |
...
| 额外请求体 | 合并到聊天请求体顶层的 JSON 对象。 | 仅用于服务商专用开关，例如 `{"enable_thinking": true}`。`model`、`messages`、`tools`、`stream`、`thinking` 等核心字段仍由 Inx 控制，且不接受 `null` 值。 |
...
| 模型能力模式 | 指定 Inx 对该 provider 使用哪种 reasoning 请求协议。 | 默认用“自动识别”。只有网关被误判，或模型文档要求特定 reasoning 格式时再切换。 |
...
| 上下文窗口 | Inx 用于自动清理上下文的 provider 级 token 预算。`0` 表示禁用自动 compaction。 | 按该 provider 的模型上下文上限填写；所选模型规格不同时使用下方的逐模型覆盖。 |

每个已选模型还提供一个可选的 **上下文窗口** 输入框。留空时继承 provider
级设置；填写正整数时只覆盖该模型。这样，同一端点下的长上下文模型不会过早
compaction，短上下文模型也不会在 Inx 清理前被服务端拒绝。
...
| 选项 | 作用 |
| --- | --- |
| 自动识别（推荐） | Inx 根据模型能力元数据和端点自动选择请求格式。 |
...
| 选项 | 作用 |
| --- | --- |
| Auto（使用服务默认） | 不写 provider 级 `thinking` 覆盖，让 Inx 使用 provider/model 默认行为。 |
...
桌面端快捷键在 **设置 → 快捷键** 中管理。选择可配置的行后按下新的组合键，Inx 会为桌面端保存该绑定。
...
| macOS `Cmd+Z`，Windows/Linux `Ctrl+Z` | 撤销输入框中的最近一次编辑 | 普通键入继续由 WebView 原生历史管理；Inx 接管的粘贴、剪切、折叠块和结构化 token 会作为完整事务恢复。 |
...
| 没有活动选区时右键 | 在本地会话粘贴剪贴板文本 | 本地会话开启鼠标接管时，Inx 只读取文本并交给正常的 bracketed-paste 处理。SSH 下远端进程无法读取本机剪贴板，请使用终端粘贴快捷键；`/mouse` 可恢复终端原生右键菜单。存在活动选区时，右键仍优先复制该选区。 |
| `/mouse` | 切换应用内鼠标接管 | 关闭后由终端处理原生拖选和右键菜单，但会失去应用内选区、滚动条和滚轮。可用 `INX_DISABLE_MOUSE=1` 让每次会话默认关闭。 |
| `Ctrl+C` | 复制、取消、清空或退出 | 有 transcript 或输入框活动选区时优先复制；否则取消运行中的 turn、清空非空输入，或在空输入下连按两次退出。 |
| `Ctrl+D` | 退出 TUI | 立即退出。 |
| 终端的文本粘贴快捷键 | 粘贴文本 | 文本保持终端原生 bracketed-paste 路径：macOS 通常是 `Cmd+V`，Linux 通常是 `Ctrl+Shift+V`，其它环境使用终端自身配置。Inx 只消费收到的文本粘贴事件，不会先探测图片。 |
...
| `/theme [auto|light|dark|style]` | 查看或切换 CLI 主题 | 不带参数会列出背景模式和命名配色。选择会保存到用户配置；单次运行可用 `INX_THEME` 和 `INX_THEME_STYLE` 覆盖。 |
...
Inx 始终会从工具子进程环境中移除已保存的 provider 与 bot 凭据变量，并自动把
全局凭据 `.env` 加入运行时禁读边界；项目 `.env` 仍保持现有的 workspace 范围行为。
**Windows 说明：**Inx 不在 Windows 上提供 OS 级 Bash 沙箱，生效模式固定为
`off`。旧配置即使写了 `bash = "enforce"` 也会解析为 `off`，`inx doctor`
...
反馈编码质量问题时，可运行 `inx doctor quality <branch-id-or-path>`（加
...
或 Discussion。它不同于 `inx doctor session`：后者生成的支持 zip 含完整未脱敏
会话，只能在可信支持渠道分享。
...
| 入口 | 用法 |
| --- | --- |
| CLI | 见上方 `inx doctor capabilities` |
| 桌面端 | **设置 → 诊断** — 刷新、复制脱敏 JSON、可选「包含当前会话运行状态」（只读活动标签 Host，**不**启动 MCP） |
| Agent | `/inx-guide`（内置 inline Skill）或自然语言描述症状；优先静态 doctor JSON，再问是否 `--live` |

退出码：`0` 允许 warning/info；`1` 表示存在 `error`（或 live 启动失败）；`2` 为参数错误。与 `inx doctor`（provider/沙箱）以及 `inx plugin doctor <name>`（单个插件包）相互独立。

## 插件（MCP）

Inx 是一个 MCP 客户端。`[[plugins]]` 的 `type` 选择传输：`stdio`（默认）启动本地子进
...
MCP 名称按 workspace 解析：项目声明覆盖同名全局安装；项目内部以 `inx.toml` 高于
...
[能力诊断](./CAPABILITY_DIAGNOSTICS.zh-CN.md)
（`inx doctor capabilities` 或 **设置 → 诊断**）。
...
**已有 Claude Code 的 `.mcp.json`？** 直接放到项目根目录，Inx 会原样读取——其
`mcpServers` 规范（`command`/`args`/`env`、`type`/`url`/`headers`、`${VAR}` 展开）
与 `[[plugins]]` 字段一一对应。两处来源会合并加载；同名时以 `inx.toml` 为准。
...
**从 `0.x` 升级？** 旧的 `~/.inx/config.json` 仍会被读取（读其 `mcpServers`、并遵从
`mcpDisabled`），作为最低优先级来源——所以 MCP 服务器照常可用；方便时再把它们挪进
`inx.toml` 的 `[[plugins]]` 或 `.mcp.json`。

## 斜杠命令

交互式 `inx` 会话里，内置命令（`/compact`、`/new`、`/clear`、`/rewind`、`/tree`、`/branch`、`/switch`、`/todo`、`/model`、`/work-mode`、`/mcp`、`/skills`、`/hooks`、`/memory`、`/goal`、`/output-style`、`/sandbox`、`/language`、`/reasoning-language`、`/help`）在本地执行——`/help` 可列出全部。
内置 **Skill**（如 `/init`、`/explore`、`/test`、`/inx-guide`）也会出现在斜杠菜单，
并可通过 `run_skill` 调用（正文按需加载；只有索引行进入缓存稳定前缀）。配置或能力排障时
用 `/inx-guide`，它会引导运行 `inx doctor capabilities`（见
[能力诊断](./CAPABILITY_DIAGNOSTICS.zh-CN.md)）。
...
是放在 `.inx/commands/`（项目）或 `~/.inx/commands/`（用户）下的 Markdown 文件——
...
另一端发现。交互式聊天里使用 `/<name> <任务>` 调用；Inx 会启动隔离子智能体，
父会话只保留任务和最终答案。

Headless CLI 提供显式管理和运行命令，同时不改变普通 `inx run` 的任务语义：

```bash
inx subagent list
inx subagent create reviewer --description "审查改动" --prompt-file reviewer.md --tools read_file,grep,bash
inx subagent edit reviewer --effort high --model deepseek-pro
inx subagent try reviewer "审查当前 diff"   # 始终只读
inx subagent run reviewer "审查并修复当前 diff"
...
- **常驻指令**来自分层加载的 `INX.md`、`AGENTS.md` 和 `CLAUDE.md`。必须在每个
...
每个真实用户回合前，Inx 会自动召回一小组相关事实。它用原始用户消息搜索，抑制“继续”
...
Goal 是长期目标的统一运行机制。普通 `/goal` 继续走轻量 Goal：Inx 会持续推进，直到
...
`.inx/autoresearch/<task-id>/` 任务目录。高级用户可以用
...
它会创建或复用项目级 `.inx/autoresearch/<task-id>/` 目录。新任务默认使用
...
`inx setup` 现在统一管理 provider、模型列表、凭据、连接测试和默认模型；所有修改
会暂存到“保存并退出”，并同步维护桌面端 provider access。完整用法见
[CLI 命令参考](./CLI.zh-CN.md#配置供应商)。若要让两个模型协同（执行器 + 规划器，
各自独立、缓存稳定的 session），向导后手动在 `inx.toml` 加一行即可：
...
Planner 会看到已加载的 `INX.md` / `AGENTS.md` 记忆，并拿到一小组只读研究工具，
因此可以先检查相关文件再把计划交给执行器。写入类和流程类工具仍只给执行器使用。

Inx 会用确定性规则路由每一轮，不再调用额外的 classifier 模型：问答、短回复、
...
Inx 会自动管理正常执行：活跃 Todo 连续 8 个工具调用轮次没有新的完成项、唯一读取、
...
`inx run --profile delivery "修复并验证这个 bug"`。Economy（轻量）初始只带 9 个工具：
...
`Shift+Tab` 切换到 Plan。Inx 先生成计划，待用户批准后工作流才切换到实施；规划期间的
...
`inx config reasoning-language auto|zh|en`。只有明确想为
