...
func TestProcessEnvUnfilteredByDefault(t *testing.T) {
	...
	if !strings.Contains(joined, "INX_TEST_SECRET_TOKEN=ghp_abcdefghijklmnopqrstuvwxyz") {
	...
	joined = strings.Join(ProcessEnv(), "\n")
	if strings.Contains(joined, "INX_TEST_SECRET_TOKEN") {
...
func TestProcessEnvAlwaysFiltersRegisteredCredentialKeys(t *testing.T) {
	const key = "INX_TEST_CUSTOM_PROVIDER_CREDENTIAL"
	t.Setenv(key, "opaque-provider-value")
	t.Setenv("INX_TEST_BENIGN_ENV", "visible")
	...
	if !strings.Contains(joined, "INX_TEST_BENIGN_ENV=visible") {
