...
func TestEnsureBlankTabInheritsActiveTabLocalSettings(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(workspace, "inx.toml"),
		[]byte(""), 0o644); err != nil {
...
func TestEnsureBlankTabUsesGlobalSessionDefaultsForModelAndToolApproval(t *testing.T) {
	isolateDesktopUserDirs(t)
	// Seed the key into Inx's user credentials store (the only source
...
// seedUserCredentials writes key=value lines into Inx's user credentials
...
func TestDesktopNewSessionDefaultsSkipsKeylessDefaultModel(t *testing.T) {
	isolateDesktopUserDirs(t)
	seedUserCredentials(t, "INX_TEST_SESSION_KEY=test-key\n")
	...
	cfg.Providers = append(cfg.Providers, config.ProviderEntry{
		...
		Model:     "test-model",
		APIKeyEnv: "INX_TEST_SESSION_KEY",
	})
	if err := cfg.SetDefaultModel("deepseek-pro/deepseek-v4-pro"); err != nil {
		t.Fatalf("SetDefaultModel: %v", err)
	}
...
func TestDesktopNewSessionDefaultsRejectsNonChatDefaultWithoutFallback(t *testing.T) {
	isolateDesktopUserDirs(t)
	seedUserCredentials(t, "INX_TEST_AUDIO_KEY=test-key\n")
...
func TestDesktopNewSessionDefaultsUsesProjectDefaultAndSkipsItsKeylessProvider(t *testing.T) {
	...
	workspace := robustTempDir(t)
	if err := os.WriteFile(filepath.Join(workspace, "inx.toml"), []byte(`default_model = "deepseek-pro/deepseek-v4-pro"
`), 0o644); err != nil {
