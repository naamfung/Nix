...
func resetTree(repo string) {
	...
	_ = exec.Command("git", "-C", repo, "clean", "-fd", "-e", "inx.toml").Run()
}
...
func renderDiff(r diffReport) string {
	...
	fmt.Fprintf(&b, "## 🤖 Inx e2e — diff test-gen (%s)\n\n", profile)
...
// repoRelFromModulePath turns "inx/internal/agent/foo.go" into
