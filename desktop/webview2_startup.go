...
const windowsWebView2StartupFallbackMessage = "The desktop interface did not become ready within 15 seconds. " +
	...
	"Check the system proxy, then restart Inx. If the problem continues, repair Microsoft Edge WebView2 Runtime or reinstall Inx.\n\n" +
	"桌面界面在 15 秒内未能就绪。不可用的 Windows 系统代理或 WebView2 故障可能阻塞了启动。" +
	"请检查系统代理后重启 Inx；如果问题仍然存在，请修复 Microsoft Edge WebView2 Runtime 或重新安装 Inx。"
...
func (a *App) startWindowsWebView2StartupFallback(ctx context.Context) {
	...
	go func() {
		...
		_, _ = runtime.MessageDialog(ctx, runtime.MessageDialogOptions{
			...
			Title:         "Inx startup delayed / Inx 启动延迟",
