...
// rejects writer-tool targets inside Inx's own session stores and makes
...
type Workspace struct {
	...
	// ManagedConfig names the Inx-owned config files the file-writers may
...
func enrichPathError(op, original string, lastErr error) error {
	...
	case err == nil && fi.IsDir():
		return pathNotExistError(op, original, fmt.Sprintf("no such file — directory %q exists, check the filename", original))
	case err == nil:
	...
	case os.IsNotExist(err):
		if anc, missing := nearestExistingAncestor(dir); anc != "" {
			return pathNotExistError(op, original, fmt.Sprintf("no such directory — nearest existing ancestor is %q, first missing segment is %q", anc, missing))
		}
