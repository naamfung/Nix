//go:build !windows && live && inx_remote_integration
...
import (
	...
	"inx/internal/remote/workbench/attach"
	workbenchclient "inx/internal/remote/workbench/client"
	"inx/internal/remote/workbench/transport"
)

const (
	remoteWorkbenchLiveGateEnv        = "INX_REMOTE_WORKBENCH_LIVE"
	remoteWorkbenchLiveProviderRefEnv = "INX_REMOTE_WORKBENCH_LIVE_PROVIDER_REF"
...
func TestRemoteWorkbenchLiveDesktopBroker(t *testing.T) {
	if os.Getenv(remoteWorkbenchLiveGateEnv) != "1" {
		t.Skip("set INX_REMOTE_WORKBENCH_LIVE=1 to run the live Desktop Broker acceptance test")
	}
	...
	t.Setenv("INX_HOME", filepath.Join(hostHome, ".inx"))
	t.Setenv("INX_STATE_HOME", filepath.Join(hostHome, ".state"))
...
func assertRemoteWorkbenchLiveHostCredentialFree(t *testing.T, hostHome, providerKeyEnv string) {
	t.Helper()
	if _, err := os.Stat(filepath.Join(hostHome, ".inx", ".env")); !os.IsNotExist(err) {
		t.Fatal("isolated Host unexpectedly has a Inx credential file")
	}
