// Package desktoplauncher implements the permanent Inx desktop entry
...
func Run(args []string, buildVersion string) int {
	if len(args) == 1 {
		...
		case "version", "--version", "-v":
			fmt.Println("inx-launcher", buildVersion)
			return 0
		case "help", "--help", "-h":
			usage()
			return 0
...
func ResolveDesktopPath(installRoot string) (string, error) {
	...
	return "", fmt.Errorf("cannot locate inx-desktop under %s (missing current.json)", installRoot)
}
...
func DetachByDefault() bool {
	...
	return name == "inx-launcher.exe" || name == "inx.exe"
}

func usage() {
	fmt.Println("usage: inx-launcher [args...]")
	fmt.Println("  Starts the active Inx desktop from current.json.")
