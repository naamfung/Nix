...
func run(args []string) int {
	...
	fs := flag.NewFlagSet("inx-update-helper", flag.ContinueOnError)
	fs.SetOutput(os.Stderr)
	fs.UintVar(&parentPID, "parent-pid", 0, "Inx process id to wait for before installing")
	...
	fs.StringVar(&installDir, "install-dir", "", "Inx installation directory")
	fs.StringVar(&relaunch, "relaunch", "", "Inx executable to start after the installer succeeds")
	fs.StringVar(&toVersion, "to-version", "", "Inx version being installed")
	...
	claimLauncher := filepath.Join(installDir, "inx-desktop.exe")
	...
	stagingDir, err := os.MkdirTemp("", "inx-update-stage-*")
	if err != nil {
...
func runVersionedWindowsUpdate(logger *log.Logger, installer, installerSHA256, installDir, relaunch, toVersion string) int {
	...
	claimed := &repair.UpdateTransaction{
		...
		ToVersion:     toVersion,
		TargetKind:    "file",
		TargetPath:    filepath.Join(installDir, "inx-desktop.exe"),
	}
...
func preferRelaunchPath(relaunch, installDir string) string {
	for _, name := range []string{"inx-launcher.exe", "Inx.exe"} {
...
func stageVerifiedInstaller(sourcePath, expectedSHA256 string) (string, func() error, error) {
	...
	dir, err := os.MkdirTemp("", "inx-update-installer-*")
	if err != nil {
		return "", nil, err
	}
	...
	defer source.Close()
	stagedPath := filepath.Join(dir, "inx-installer.exe")
...
func newLogger() *log.Logger {
	...
	if err == nil {
		dir = filepath.Join(dir, "Inx", "updates")
...
func runInstaller(installer, installDir string) error {
	...
	// visible. /INXSTAGE makes the signed installer extract only; the helper
...
func cleanupOwnedWindowsUpdateDirectory(path string, owner os.FileInfo) error {
	...
	for attempt := 0; attempt < 16; attempt++ {
		cleanup := fmt.Sprintf("%s.inx-cleanup-%d-%d", path, time.Now().UTC().UnixNano(), attempt)
