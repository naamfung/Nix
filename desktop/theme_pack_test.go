...
func TestThemePackV2IndependentSceneImagesRoundTrip(t *testing.T) {
	...
	exportPath := filepath.Join(t.TempDir(), "two-scenes.inx-theme")
...
func TestImportExportRoundTrip(t *testing.T) {
	...
	exportPath := filepath.Join(t.TempDir(), "out.inx-theme")
	if err := exportThemePackZIP(m.ID, exportPath); err != nil {
		t.Fatal(err)
	}
...
func TestSameIDImportRequiresReplace(t *testing.T) {
	...
	exportPath := filepath.Join(t.TempDir(), "v2.inx-theme")
	m.Name = "V2"
