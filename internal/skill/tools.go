...
func (t *installSkillTool) Description() string {
	scope := "'global' (only option — no project workspace) writes to the Inx home skills directory."
	if t.store.HasProjectScope() {
		scope = "'project' (default) writes to <repo>/.inx/skills/ (this workspace only); 'global' writes to the Inx home skills directory (every project)."
	}
...
"type":"object",
"properties":{
  "name":{"type":"string","description":"Identifier — letters/digits/_/-/., 1-64 chars, starts alphanumeric. Becomes the skill folder name under the selected skills directory."},
