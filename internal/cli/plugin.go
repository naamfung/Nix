...
func pluginUsage() {
  ...
  inx plugin install <source> [--yes] [--dry-run] [--link] [--replace]
  inx plugin list
  inx plugin show <name>
  inx plugin enable <name>
  inx plugin disable <name>
  inx plugin remove <name>
  inx plugin doctor <name>`)
}
...
func pluginSetEnabledCommand(args []string, enabled bool) int {
	...
	if err := pluginpkg.SetEnabled(config.InxHomeDir(), args[0], enabled); err != nil {
		fmt.Fprintln(os.Stderr, err)
		return 1
	}
