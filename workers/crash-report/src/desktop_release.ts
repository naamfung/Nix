const R2_BASE = "https://dl.inx.io";
const GITHUB_RELEASES_API = "https://api.github.com/repos/naamfung/inx/releases?per_page=100";
const GITHUB_LATEST_RELEASE_API = "https://api.github.com/repos/naamfung/inx/releases/latest";
...
function normalizeCLIRelease(
  ...
  const releaseURL = `https://github.com/naamfung/inx/releases/tag/${tag}`;
  return {
    order,
    release: {
      tag_name: tag,
...
async function fetchCLIRelease(url: string, channel: PublicReleaseChannel, source: string): Promise<Response | null> {
  try {
    const response = await fetch(url, {
      headers: { accept: "application/json", "user-agent": "inx-release-gateway" },
    });
...
async function fetchLatestCLIReleaseFromGitHub(channel: PublicReleaseChannel): Promise<Response | null> {
  try {
    const response = await fetch(GITHUB_RELEASES_API, {
      headers: { accept: "application/vnd.github+json", "user-agent": "inx-release-gateway" },
    });
    if (!response.ok) return null;
...
function desktopAssetBases(
  ...
  return [r2, `https://github.com/naamfung/inx/releases/download/desktop-${version}/`];
}
  ...
  try {
    ...
    const res = await fetch(safeURL, {
      headers: {
        ...
        "user-agent": "inx-release-gateway",
      },
    });
...
export async function handleCLIRelease(channel: PublicReleaseChannel): Promise<Response> {
  ...
  const cacheKey = new Request(`https://crash.inx.io/v1/cli/releases/${channel}/latest.json`);
