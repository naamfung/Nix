// Package attach implements `inx remote attach-workspace --stdio`.
...
type Options struct {
	...
	SchemaHash string
	// RuntimeBinary is optional path to inx for launching runtime child.
...
func ensureRuntime(ctx context.Context, opts Options, home, workspace, sock string) error {
	...
	// Detached child: inx remote-runtime-workbench --workspace --socket
