...
func (t *installSourceTool) pluginPackageAction(req request, pkg pluginpkg.Package, source string) (action, error) {
	...
	root := ""
	if t.inxHome != "" {
		root = pluginpkg.InstallRoot(t.inxHome, name)
	}
	skills, commands, hooks, mcp := pkg.CapabilityCounts()
	agents := pkg.Inventory().Agents
	if pkg.ManifestKind != "inx" && skills+commands+hooks+mcp+len(agents) == 0 {
		return action{}, newErr(ErrNoCompatibleCapabilities, "plugin %q has no Inx-compatible capabilities; skipped: %v", name, pkg.Compatibility.Skipped)
	}
	...
	a := action{
		...
		ConfigPath:          pluginpkg.StatePath(t.inxHome),
	...
	if hooks > 0 {
		...
		a.RiskReasons = append(a.RiskReasons, "registers shell hooks that execute during Inx sessions")
	}
	if mcp > 0 {
		a.RiskLevel = RiskHigh
...
func (t *installSourceTool) applyInstallPluginPackage(ctx context.Context, req request, act *action) error {
	if t.inxHome == "" {
		return newErr(ErrSourceUnreadable, "plugin install requires a Inx home directory")
	}
	if !pluginpkg.IsValidName(act.Name) {
		return newErr(ErrInvalidManifest, "invalid plugin name %q", act.Name)
	}
	target := pluginpkg.InstallRoot(t.inxHome, act.Name)
	...
	if pkg.ManifestKind != "inx" {
		...
		if skills+commands+hooks+mcp+pkg.AgentCount() == 0 {
			return newErr(ErrInvalidManifest, "plugin %q no longer has any Inx-compatible capabilities", act.Name)
		}
	}
	...
	installed := pluginpkg.InstalledPlugin{
		Name:         act.Name,
		Source:       act.Source,
		Root:         pluginpkg.RelativeRoot(t.inxHome, target),
		Version:      pkg.Manifest.Version,
	...
	if err := pluginpkg.Upsert(t.inxHome, installed); err != nil {
		return err
	}
	act.Target = target
...
func (t *installSourceTool) preparePluginSource(ctx context.Context, source, mode string) (string, string, func(), error) {
	...
	if isURL(source) {
		...
		tmp, err := os.MkdirTemp("", "inx-plugin-*")
		if err != nil {
			return "", "", func() {}, err
		}
...
func (t *installSourceTool) applyRemovePluginPackage(_ request, act *action) error {
	installed, ok, err := pluginpkg.Remove(t.inxHome, act.Name)
	if err != nil || !ok {
		return err
	}
	root := pluginpkg.ResolveRoot(t.inxHome, installed.Root)
	...
	pluginsDir := pluginpkg.PluginsDir(t.inxHome)
