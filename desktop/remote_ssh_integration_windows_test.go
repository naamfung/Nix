//go:build windows && inx_legacy_remote_integration
...
import (
	...
	remoteclient "inx/internal/remote/client"
	"inx/internal/remote/protocol"
)

const (
	remoteSSHIntegrationGateEnv       = "INX_REMOTE_SSH_INTEGRATION"
	remoteSSHIntegrationSSHPathEnv    = "INX_REMOTE_SSH_INTEGRATION_SSH_PATH"
	remoteSSHIntegrationConfigPathEnv = "INX_REMOTE_SSH_INTEGRATION_CONFIG_PATH"
	remoteSSHIntegrationKnownHostsEnv = "INX_REMOTE_SSH_INTEGRATION_KNOWN_HOSTS_PATH"
	remoteSSHIntegrationHostAliasEnv  = "INX_REMOTE_SSH_INTEGRATION_HOST_ALIAS"
	remoteSSHIntegrationTimeout       = 75 * time.Second
	remoteSSHIntegrationGapWait       = 7 * time.Second
	remoteSSHIntegrationTopicPrefix   = "Inx Remote SSH reconnect integration"
	remoteSSHIntegrationBeforeMarker  = "inx-remote-event-before"
	remoteSSHIntegrationGapMarker     = "inx-remote-event-gap"
	remoteSSHIntegrationAfterMarker   = "inx-remote-event-after"
	remoteSSHIntegrationShellCommand  = "printf 'inx-remote-event-before\\n'; sleep 5; printf 'inx-remote-event-gap\\n'; sleep 30; printf 'inx-remote-event-after\\n'"
)
...
// ssh.exe -> sshd -> inx remote attach-workspace --stdio -> daemon path. OpenSSH,
...
func TestRemoteSSHConfigAttachReconnectIntegration(t *testing.T) {
	if os.Getenv(remoteSSHIntegrationGateEnv) != "1" {
		t.Skip("set INX_REMOTE_SSH_INTEGRATION=1 to run the real SSH integration test")
	}
...
func remoteSSHIntegrationHasRemoteCode(err error, code protocol.InxErrorCode) bool {
