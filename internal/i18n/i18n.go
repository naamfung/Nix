...
type Messages struct {
	...
	StepScaffold    string // step 1 desc — inx setup
	StepSetKey      string // step 2 command label

	// `inx init` — points to the in-session /init skill + setup
	InitHint       string
	StepSetKeyHint string // step 2 desc — env var hint
	StepChatDesc   string // interactive session step desc
	StepRunDesc    string // inx run step desc
	HelpFooter     string // dim footer linking to inx help
	...
	ApprovalToolLabelConfigWrite           string // user-facing label for Inx-managed config write approvals
	...
	WroteFileFmt             string // "Wrote %s" — used for inx.toml and .env both
	...
	SkipStaleCustomEntryFmt    string // "skipping stale %q entry from inx.toml (pointing at %s) — please remove it"
	...
	RemoteNoHostsHint         string // "no remote hosts configured; add one with `inx remote add`"

	// top-level / runAgent
	UnknownCommandFmt         string // "unknown command %q"
	UsageRunHint              string // "usage: inx run [--model NAME] <task>"
	...
	// `inx upgrade` / `inx update` — self-update
	...
	// `inx report` — local CLI crash review and explicit upload
...
//
// Priority: override > INX_LANG > LC_ALL > LC_MESSAGES > LANG > "en".
...
func envCandidates() []string {
	keys := []string{"INX_LANG", "LC_ALL", "LC_MESSAGES", "LANG"}
