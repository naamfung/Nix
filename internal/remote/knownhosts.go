...
// known_hosts files (read-only) and a Inx-managed file (read-write, TOFU).
type HostKeyPolicy struct {
	...
	// ManagedPath is the Inx-managed known_hosts file that accepted TOFU
...
func (hostKeyLookupProbe) Type() string    { return "inx-host-key-lookup-probe" }
func (hostKeyLookupProbe) Marshal() []byte { return []byte("inx-host-key-lookup-probe") }
