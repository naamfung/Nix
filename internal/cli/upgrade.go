...
func parseCLIReleaseChannel(value string) (cliReleaseChannel, error) {
	...
	default:
		return "", fmt.Errorf("release channel %q is unsupported; Inx now uses the official release", value)
	}
}
...
// upgradeCommand handles `inx upgrade` (and `inx update`).
func upgradeCommand(args []string, version string) int {
	...
	base := fmt.Sprintf("inx-%s-%s", runtime.GOOS, runtime.GOARCH)
...
func cliPlatformAssetName(goos, goarch string) string {
	...
	return fmt.Sprintf("inx-%s-%s%s", goos, goarch, suffix)
}
...
// extractBinary pulls the "inx" binary from a .tar.gz or .zip archive.
...
// rename it aside to .inx.old first, then place the new binary.
