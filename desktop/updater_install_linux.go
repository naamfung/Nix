...
func dirIsWritable(dir string) bool {
	...
	f, err := os.CreateTemp(dir, ".inx-write-test-*")
	if err != nil {
		return false
	}
	name := f.Name()
...
//
//	deb      — dpkg owns the absolute executable path as inx-desktop, and the
...
func detectLinuxInstallProfile() installProfile {
	...
	return installProfile{
		Mode:          installModeManual,
		CanSelfUpdate: false,
		ManualReason:  "this install is not writable and is not managed by the inx-desktop package; download the package from the download page",
	}
}

// isDpkgOwnedInx reports whether absolute path belongs to the inx-desktop
// package. Uses absolute dpkg-query and requires both package name and path match.
func isDpkgOwnedInx(absPath string) bool {
	...
	// Output shape: "inx-desktop: /usr/bin/inx-desktop"
