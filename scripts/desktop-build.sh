...
#   macOS:   Inx-darwin-<arch>.zip                  (ditto archive; updater channel)
#            Inx-darwin-universal.dmg               (drag-to-install; human download)
#   Windows: Inx-windows-<arch>-installer.exe       (NSIS per-user installer; updater channel)
#            Inx-windows-<arch>.zip                 (portable human download)
#   Linux:   Inx-linux-<arch>.tar.gz                (desktop + guard + CLI; portable updater)
#            Inx-linux-<arch>.deb                   (Debian/Ubuntu package; native updater)
#
...
APPNAME="Inx"            # wails.json productName -> Inx.app
BINNAME="inx-desktop"    # wails.json outputfilename -> linux binary name
CLINAME="inx"            # bundled CLI sidecar used for remote serve upload
WINDOWS_CLINAME="inx-cli" # Windows cannot store Inx.exe and inx.exe separately
GUARDNAME="inx-guard"
LAUNCHERNAME="inx-launcher"
...
source_revision_ldflag="-X inx/internal/remote/protocol.linkedSourceRevision=$SOURCE_REVISION"
...
# build_guard produces the one-shot legacy migrator still named inx-guard
...
build_guard() {
	echo "==> go build Inx legacy migrator (compat name inx-guard)"
	mkdir -p "$(dirname "$guard_out")"
	if [ "$arch" = universal ]; then
		guard_tmp=$(mktemp -d)
		(cd "$ROOT" && GOOS=darwin GOARCH=amd64 CGO_ENABLED=0 go build -trimpath -ldflags="-s -w -X main.version=$VERSION" -o "$guard_tmp/amd64" ./cmd/inx-legacy-migrator)
		(cd "$ROOT" && GOOS=darwin GOARCH=arm64 CGO_ENABLED=0 go build -trimpath -ldflags="-s -w -X main.version=$VERSION" -o "$guard_tmp/arm64" ./cmd/inx-legacy-migrator)
		lipo -create "$guard_tmp/amd64" "$guard_tmp/arm64" -output "$guard_out"
		rm -rf "$guard_tmp"
	else
		(cd "$ROOT" && GOOS="$os" GOARCH="$arch" CGO_ENABLED=0 go build -trimpath -ldflags="-s -w -X main.version=$VERSION" -o "$guard_out" ./cmd/inx-legacy-migrator)
	fi
}

build_cli() {
	echo "==> go build Inx CLI sidecar"
	mkdir -p "$(dirname "$cli_out")"
	if [ "$arch" = universal ]; then
		cli_tmp=$(mktemp -d)
		(cd "$ROOT" && GOOS=darwin GOARCH=amd64 CGO_ENABLED=0 go build -trimpath -ldflags="-s -w -X main.version=$VERSION $source_revision_ldflag" -o "$cli_tmp/amd64" ./cmd/inx)
		(cd "$ROOT" && GOOS=darwin GOARCH=arm64 CGO_ENABLED=0 go build -trimpath -ldflags="-s -w -X main.version=$VERSION $source_revision_ldflag" -o "$cli_tmp/arm64" ./cmd/inx)
		lipo -create "$cli_tmp/amd64" "$cli_tmp/arm64" -output "$cli_out"
		rm -rf "$cli_tmp"
	else
		(cd "$ROOT" && GOOS="$os" GOARCH="$arch" CGO_ENABLED=0 go build -trimpath -ldflags="-s -w -X main.version=$VERSION $source_revision_ldflag" -o "$cli_out" ./cmd/inx)
	fi
}
...
if [ "$os" = windows ]; then
	...
	windows_resource_tool="$windows_resource_tool_dir/inx-windows-resource.exe"
	...
	build_guard
	stamp_windows_executable "$guard_out" "Inx Legacy Migrator" "$GUARDNAME" "$GUARDNAME.exe"
	...
	(cd "$ROOT" && GOOS=windows GOARCH="$arch" CGO_ENABLED=0 go build -trimpath \
		-ldflags="-s -w -H windowsgui -X main.version=$VERSION" -o "$launcher_out" ./cmd/inx-launcher)
	stamp_windows_executable "$launcher_out" "Inx Launcher" "$LAUNCHERNAME" "$LAUNCHERNAME.exe"
	...
	stamp_windows_executable "build/windows/installer/$UPDATE_HELPER" "Inx Update Helper" "inx-update-helper" "$UPDATE_HELPER"
	cli_out="$ROOT/desktop/build/windows/installer/$WINDOWS_CLINAME.exe"
	build_cli
	stamp_windows_executable "$cli_out" "Inx CLI" "$WINDOWS_CLINAME" "$WINDOWS_CLINAME.exe"
	...
	rm -f "build/windows/installer/inx-uninstall.exe"
fi
build_args=()
...
if [ "$os" != windows ]; then
	# Linux still ships a one-shot migrator named inx-guard in the portable
	...
	if [ "$os" = linux ]; then
		...
		(cd "$ROOT" && GOOS=linux GOARCH="$arch" CGO_ENABLED=0 go build -trimpath \
			-ldflags="-s -w -X main.version=$VERSION" -o "$launcher_out" ./cmd/inx-launcher)
	fi
...
case "$os" in
darwin)
	# Wails names the bundle after outputfilename (inx-desktop.app); repackage
	# it as Inx.app for a clean user-facing name.
	staging=$(mktemp -d)
	app="$staging/${APPNAME}.app"
	cp -R "build/bin/inx-desktop.app" "$app"
...
linux)
	for desktop_contract in \
		'Exec=inx-launcher' \
		'Icon=inx-desktop' \
		'StartupWMClass=inx-desktop'; do
		grep -F -x -q "$desktop_contract" build/linux/inx.desktop || { echo "Linux desktop entry missing: $desktop_contract" >&2; exit 1; }
	done
	# Portable Linux tarball: desktop + thin launcher + one-shot migrator
	# (compat name inx-guard) + CLI. After migrator runs, Guard self-deletes.
	...
	dpkg-deb --field "$deb_path" Package | grep -x 'inx-desktop' >/dev/null
	dpkg-deb --field "$deb_path" Version | grep -x "$deb_version" >/dev/null
	dpkg-deb --field "$deb_path" Depends | grep -F 'pkexec' >/dev/null
	dpkg-deb --contents "$deb_path" | grep -E 'usr/lib/inx/inx-update-helper' >/dev/null
	dpkg-deb --contents "$deb_path" | grep -E 'usr/share/polkit-1/actions/io.inx.desktop.update.policy' >/dev/null
	;;
*)
