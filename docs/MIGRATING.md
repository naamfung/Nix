# Migrating to Inx 1.0 (the Go rewrite)

Inx 1.0 is a **ground-up rewrite in Go**. It is a new codebase, not an
...
**`npm i -g inx` installs the current official `1.x` release.** npm's `latest` tag
...
```sh
npm i -g inx          # current official 1.x release
npm i -g inx@0.53.2   # pin the legacy TS build
```

Prebuilt archives (`inx-<os>-<arch>.tar.gz` / `.zip`) and the desktop
...
| Legacy | Inx 1.0 |
|---|---|
| TS config files | `inx.toml` (project) / `config.toml` in Inx home (`~/.inx/` on macOS/Linux; `%AppData%\inx\` on Windows) from v1.8.1 — see `inx.example.toml` and [Configuration paths](./CONFIG_PATHS.md) |
| env / API keys | Provider config keeps `api_key_env`; saved key values live in Inx home `.env` (`DEEPSEEK_API_KEY`, `MIMO_API_KEY`, …) |
| project memory | `INX.md` (+ auto-memory), Claude-Code-compatible |
| MCP servers | `[[plugins]]` in `inx.toml`, or a Claude-Code `.mcp.json` (read as-is) |

On first launch, v1.8.1+ runs a one-time, **non-destructive** import: it reads
legacy config from `~/Library/Application Support/inx/config.toml`,
`~/.config/inx/config.toml`, `~/.inx/inx.toml`, or v0.x
`~/.inx/config.json` (API key, base URL, language, MCP servers), migrates
legacy credentials into `<Inx home>/.env` when a key is missing there, and
imports past sessions from legacy session directories. Old files are left
untouched, and Inx prints a boot notice when it imports data. Each session lands in the
...
The command is available only in Go-based Inx builds that include it; if you
...
`/migrate --from "D:\OldInx"` to import sessions from that explicit source.
See
...
| --- | --- |
| `INX.md`, `AGENTS.md`, `CLAUDE.md` | Loaded as standing instructions with source, directory, precedence, imports, and diagnostics. Existing file names remain valid. |
...
changing the fact body. Compatibility routing fields keep older Inx
...
- **MCP setup is now add-and-use.** Servers added by the user (Desktop, CLI,
  ...
  `inx.toml` / `.mcp.json` servers stay project-scoped and are trusted
  without a separate launch confirmation. Project entries override same-name
  global entries; `inx.toml` overrides `.mcp.json` inside the project.
  Treat opening an unfamiliar repository as opting into executable project
  configuration: review `.inx/settings.json`, `inx.toml`, and
  `.mcp.json` before starting Inx. If a repository causes unexpected MCP
...
- **Web dashboard remains available; desktop is recommended**: run
  `inx serve` when a local browser UI is useful. For the primary visual
...
Inx 1.0 supports reading and editing files in UTF-8, UTF-8 BOM, UTF-16
