...
// the v0.x ~/.inx/config.json's mcpServers. Provider api_key_env values
// resolve from Inx's global .env, not from project .env files.
...
// each project's inx.toml + .mcp.json are resolved independently without
// changing the process cwd, while provider keys stay rooted in Inx home.
//
...
// reads project inx.toml files and never performs on-disk migrations.
...
func loadForRoot(root string, migrateOnDisk bool) (*Config, error) {
	...
	// A repository-local inx.toml must never switch that global choice.
	cfg.CLI = globalCLI
	// Secret protection is a user-global security control: a cloned repo's
	// inx.toml must not be able to flip on the workflow-breaking env/path
	// protections.
	cfg.Secrets = globalSecrets
	// Remote SSH hosts are equally user-global: a cloned repo's inx.toml
	// must not be able to inject hosts, jump chains, or port forwards that
	// steer where Inx opens connections.
	...
	// project inx.toml doesn't drop the global config's MCP servers.
	...
	// Project inx.toml wins on a name collision; project .mcp.json wins
	...
	// ~/.inx/config.json's mcpServers. Once the migration marker exists, the
...
// default_model when a project inx.toml overrides it with a reference no
// configured provider serves (#4218). Pre-v1.11 persistence paths (e.g. the
// "always allow" writer) full-rendered ./inx.toml and pinned the built-in
...
// LoadForEdit returns a config to seed the `inx setup` wizard when reconfiguring:
...
// of resetting to defaults. Inx's global .env is loaded so api_key_env
...
// MiMo providers needed by legacy refs that live outside inx.toml, such as
