...
// Run with:
//
//	INX_LIVE_CHROME_MCP=1 go test ./internal/plugin \
//	  -run '^TestChromeDevtoolsMCPLive$' -v -count=1 -timeout=3m
func TestChromeDevtoolsMCPLive(t *testing.T) {
	if os.Getenv("INX_LIVE_CHROME_MCP") != "1" {
		t.Skip("set INX_LIVE_CHROME_MCP=1 to run the real Chrome MCP smoke test")
	}
	...
	pageHTML := `data:text/html,<title>Inx%20MCP</title><h1>Inx%20MCP%20Ready</h1>`
	...
	out = executeLiveChromeTool(t, callCtx, tools, "wait_for", map[string]any{"text": []string{"Inx MCP Ready"}, "timeout": 10_000})
	if !strings.Contains(out, "Inx MCP Ready") {
	...
	out = executeLiveChromeTool(t, callCtx, tools, "evaluate_script", map[string]any{
		"function": `() => { console.log("inx-mcp-console"); return document.title; }`,
	})
	if !strings.Contains(out, "Inx MCP") {
	...
	if !strings.Contains(consoleOut, "inx-mcp-console") || strings.TrimSpace(networkOut) == "" {
