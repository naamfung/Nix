...
func NewChromium() *Chromium {
	// Inx's WebView only loads embedded assets and loopback remote-workspace
	// pages. Keep that native UI independent from a stale Windows system proxy;
	// provider, updater, MCP, and SSH traffic use Inx's separate Go client.
	e := &Chromium{AdditionalBrowserArgs: []string{inxNoProxyServerBrowserArg}}
	/*
