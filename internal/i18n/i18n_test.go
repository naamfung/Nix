...
// $LC_ALL / $INX_LANG. Unknown locales return "" so DetectLanguage falls
...
// TestDetectLanguagePriority verifies override beats env and that INX_LANG
...
func TestDetectLanguagePriority(t *testing.T) {
	...
	t.Setenv("INX_LANG", "en")
	if got := DetectLanguage(""); got != "en" {
		t.Errorf("INX_LANG=en overriding LANG=zh: got %q, want en", got)
	}
