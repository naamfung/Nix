...
func TestManagedConfigWriteFailsClosedWithoutApprover(t *testing.T) {
	...
	cfg := config.Default()
	managed := NewManagedConfigPaths(config.InxManagedConfigPaths())
...
func TestManagedConfigWriteGatedOnApprover(t *testing.T) {
	...
	decline := &stubConfigWriteApprover{allow: false, reason: "the user declined this Inx config write"}
	...
	// Even with an always-allowing approver, non-config files in the Inx
	...
	for _, target := range []string{
		...
		filepath.Join(home, ".inx", ".env"),
		filepath.Join(home, ".inx", "settings.json"),
		filepath.Join(home, ".inx", "skills", "evil", "SKILL.md"),
	} {
...
func TestBashSandboxConfinement(t *testing.T) {
	...
	work, err := os.MkdirTemp(home, ".inx-bashsb-*")
	if err != nil {
	...
	outPath := filepath.Join(home, ".inx-bashsb-escape.txt")
