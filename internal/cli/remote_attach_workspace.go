...
import (
	...
	"inx/internal/remote/workbench/runtime"
)

// remoteAttachWorkspaceCLI runs `inx remote attach-workspace --stdio`.
// An explicit --workspace / INX_ATTACH_WORKSPACE binds the target; when
...
func remoteAttachWorkspaceCLI(args []string, version string) int {
	...
	if !stdio {
		fmt.Fprintln(os.Stderr, "usage: inx remote attach-workspace --stdio [--workspace <path>]")
		return 2
	}
	if workspace == "" {
		workspace = strings.TrimSpace(os.Getenv("INX_ATTACH_WORKSPACE"))
	}
...
func remoteRuntimeWorkbenchCLI(args []string, version string) int {
	...
	if workspace == "" || socket == "" {
		fmt.Fprintln(os.Stderr, "usage: inx remote runtime-workbench --workspace <path> --socket <path>")
		return 2
	}
