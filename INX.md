...
agent. It is the Inx analog of Claude Code's CLAUDE.md.
...
- Standing instructions are hierarchical: committed/shared `INX.md`,
  ...
  (`INX_STATE_HOME`, otherwise `INX_HOME`, otherwise `~/.inx` on
  macOS/Linux or `%APPDATA%\inx` on Windows). All distinct supported files
