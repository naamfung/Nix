// Package bootstrap starts and manages a detached `inx serve` process on
// a remote host over an established SSH connection. It detects the remote
// OS/arch, locates or installs inx, launches serve bound to a random
// loopback port with a file-based token (never in argv), and records the
// result under the remote ~/.inx/remote so a later reconnect can reuse
...
type Options struct {
	...
	LocalBinary    string                                                        // path to the running inx binary, for same-platform upload
...
func EnsureServe(ctx context.Context, conn Conn, opts Options) (Result, error) {
	...
	// 3. Locate or install a usable inx.
	...
	if pid <= 0 || !pidIsServe(ctx, conn, pid, paths) {
		cleanupFailedLaunch(conn, fs, paths, pid)
		return Result{}, errors.New("bootstrap: launched process did not become the expected inx serve")
	}
...
// pidIsServe reports whether pid is running AND is a inx serve process,
