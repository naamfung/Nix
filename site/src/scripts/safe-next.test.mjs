...
// allowed inx.io host. Asserting this (rather than a fixed string) catches
...
function assertLandsSomewhereSafe(returned) {
  ...
  const ok = landed.origin === ORIGIN || landed.host === "inx.io" || landed.host.endsWith(".inx.io");
...
test("same-origin path passes through as an absolute same-origin URL", () => {
  ...
  assert.equal(got, "https://inx.io/account/settings?x=1#y");
  assertLandsSomewhereSafe(got);
});

test("allowed inx.io subdomain passes through", () => {
  const got = safeNext("https://crash.inx.io/x", ORIGIN);
  assert.equal(got, "https://crash.inx.io/x");
  assertLandsSomewhereSafe(got);
});
...
test("dot-segment inputs that normalize to a //host pathname do not escape", () => {
  ...
  // returning the absolute href keeps them on inx.io. Assert end to end.
...
test("rejects a inx.io-lookalike host", () => {
  assert.equal(safeNext("https://inx.io.evil.example/", ORIGIN), null);
});
