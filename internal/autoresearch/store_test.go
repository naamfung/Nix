...
func TestCreateTaskCreatesHostOwnedLayoutAndInitialState(t *testing.T) {
	...
	wantRoot := filepath.Join(root, ".inx", "autoresearch", task.ID)
...
func TestRemoveTaskByCreateTokenRemovesIncompleteReservation(t *testing.T) {
	...
	taskRoot := filepath.Join(root, ".inx", "autoresearch", taskID)
...
func TestLoadTaskRejectsSymlinkTaskDirectory(t *testing.T) {
	...
	if err := os.MkdirAll(filepath.Join(root, ".inx", "autoresearch"), 0o755); err != nil {
		t.Fatalf("create autoresearch root: %v", err)
	}
	if err := os.Symlink(outside, filepath.Join(root, ".inx", "autoresearch", taskID)); err != nil {
...
func TestResumeFromGoalTextLoadsExplicitTaskPath(t *testing.T) {
	...
	resumed, ok, err := store.ResumeFromGoalText("继续 .inx/autoresearch/" + task.ID + "/ 这个任务")
	if err != nil {
