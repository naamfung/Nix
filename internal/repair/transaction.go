...
func validateRepairChange(change RepairChange) error {
	...
	case change.Scope == "project":
		if filepath.Base(target) != "inx.toml" {
	...
	if filepath.Dir(previous) == filepath.Dir(target) && strings.HasPrefix(filepath.Base(previous), filepath.Base(target)+".inx-") {
		return nil
	}
...
func UndoLastRepair() (*RepairTransaction, error) {
	...
	for i := len(tx.Changes) - 1; i >= 0; i-- {
		...
		if _, err := os.Lstat(change.TargetPath); err == nil {
			...
			redo = fmt.Sprintf("%s.inx-redo-%s-%d", change.TargetPath, now.Format("20060102T150405.000000000Z"), i)
