...
function registryBindings(env: Env): RegistryBindings {
  return {
    ...
    ACCOUNTS_ORIGIN: env.ID_ORIGIN ?? "https://id.inx.io",
    APP_ORIGIN: env.APP_ORIGIN ?? "https://inx.io",
    ALLOWED_ORIGINS: env.ALLOWED_ORIGINS ?? "https://inx.io,https://www.inx.io",
  };
}
...
async function runIngestSentinel(env: Env): Promise<void> {
  ...
  const message = `crash.inx.io ingest sentinel: ${problems.join("; ")} — https://crash.inx.io/stats`;
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    // Authentication moved to id.inx.io; these paths just bounce there.
