...
func TestFileUpdateRollbackRestoresPreviousBinary(t *testing.T) {
	...
	target := filepath.Join(t.TempDir(), "inx-desktop")
	originalExecutable := repairExecutable
	repairExecutable = func() (string, error) { return filepath.Join(filepath.Dir(target), "inx-guard"), nil }
...
func TestAppBundleRollbackRestoresWhenLiveBundleIsMissing(t *testing.T) {
	...
	app := filepath.Join(dir, "Inx.app")
	exe := filepath.Join(app, "Contents", "MacOS", "Inx")
...
func TestMarkUpdateHealthyRejectsAppBundleDrift(t *testing.T) {
	...
	backup := app + ".inx-update-backup"
	staging, err := os.MkdirTemp("", "inx-mac-update-*")
	if err != nil {
		t.Fatal(err)
	}
	t.Cleanup(func() { _ = os.RemoveAll(staging) })
	stagedApp := filepath.Join(staging, "Inx.app")
	stagedExe := filepath.Join(stagedApp, "Contents", "MacOS", "Inx")
...
func TestAppBundleRollbackRejectsChangedBackupTree(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(backup, "Contents", "MacOS", "Inx"), []byte("tampered"), 0o700); err != nil {
		t.Fatal(err)
	}
...
func TestAppBundleRollbackCompensatesBackupChangedDuringPublish(t *testing.T) {
	...
	rollbackSwapRename = func(oldPath, newPath string) error {
		...
		if oldPath == backup && newPath == app {
			return os.WriteFile(filepath.Join(app, "Contents", "MacOS", "Inx"), []byte("tampered-during-publish"), 0o700)
		}
		return nil
	}
	...
	rejected, err := filepath.Glob(app + ".inx-cleanup-*")
	if err != nil || len(rejected) != 1 {
		t.Fatalf("rejected backup bundle = %v, %v", rejected, err)
	}
	rejectedExe := filepath.Join(rejected[0], "Contents", "MacOS", "Inx")
...
func TestLegacyAppBundleRollbackWithoutTreeDigest(t *testing.T) {
	...
	backupExe := filepath.Join(backup, "Contents", "MacOS", "Inx")
...
func TestRepairPlanRejectsLegacyAppBundleSymlinkBackupBeforeMutation(t *testing.T) {
	...
	if got, err := os.ReadFile(filepath.Join(external, "Contents", "MacOS", "Inx")); err != nil || string(got) != "old" {
...
func TestRollbackPendingUpdateRejectsUnexpectedVersion(t *testing.T) {
	...
	target := filepath.Join(dir, "inx-desktop")
	originalExecutable := repairExecutable
	repairExecutable = func() (string, error) { return filepath.Join(dir, "inx-guard"), nil }
	t.Cleanup(func() { repairExecutable = originalExecutable })
...
func TestRollbackPendingUpdateFailsClosedWhenStateLockIsUnavailable(t *testing.T) {
	...
	target := filepath.Join(dir, "inx-desktop")
	guard := filepath.Join(dir, "inx-guard")
	originalExecutable := repairExecutable
...
func TestFileUpdateRollbackRestoresReleaseUnit(t *testing.T) {
	...
	missingSibling := filepath.Join(dir, "inx-update-helper.exe")
...
func TestClearUpdateApplyFailureExactPreservesDifferentTransactionMarker(t *testing.T) {
	...
	first := &UpdateTransaction{
		...
		TargetKind:    "file",
		TargetPath:    filepath.Join(t.TempDir(), "inx-desktop"),
...
func TestRecordClaimedFileUpdateInstalledBindsOptionalMissingSibling(t *testing.T) {
	...
	target := filepath.Join(dir, "inx-desktop")
	guard := filepath.Join(dir, "inx-guard")
	optionalCLI := filepath.Join(dir, "inx")
	originalExecutable := repairExecutable
	repairExecutable = func() (string, error) { return guard, nil }
	t.Cleanup(func() { repairExecutable = originalExecutable })
	for path, body := range map[string]string{target: "old-desktop", guard: "old-guard"} {
...
func TestHealthyAppUpdateRejectsBackupDrift(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(backup, "Contents", "MacOS", "Inx"), []byte("unrelated"), 0o700); err != nil {
		t.Fatal(err)
	}
	...
	if got, err := os.ReadFile(filepath.Join(backup, "Contents", "MacOS", "Inx")); err != nil || string(got) != "unrelated" {
...
func TestFileUpdateRollbackCompensatesOnPartialFailure(t *testing.T) {
	...
	target := filepath.Join(dir, "inx-desktop")
	guard := filepath.Join(dir, "inx-guard")
	added := filepath.Join(dir, "inx-update-helper.exe")
	originalExecutable := repairExecutable
	repairExecutable = func() (string, error) { return guard, nil }
	t.Cleanup(func() { repairExecutable = originalExecutable })
	...
	rollbackPublishStage = func(oldpath, newpath string) error {
		if strings.HasPrefix(oldpath, guard+".inx-rollback-stage-") && newpath == guard {
	...
	leftovers, err := filepath.Glob(filepath.Join(dir, "*.inx-rollback-*"))
...
func TestFileUpdateRollbackPreservesAsideReplacedBeforeCleanup(t *testing.T) {
	...
	rollbackPublishStage = func(oldpath, newpath string) error {
		...
		return os.WriteFile(newpath+".inx-rollback-aside", []byte("concurrent-aside"), 0o700)
	}
	t.Cleanup(func() { rollbackPublishStage = originalPublish })
	...
	aside := target + ".inx-rollback-aside"
...
func TestFileUpdateRollbackRetryPreservesRetainedNewBinary(t *testing.T) {
	...
	targetAside := target + ".inx-rollback-aside"
...
func TestFileUpdateRollbackRechecksWholeReleaseUnitBeforeCommit(t *testing.T) {
	...
	injected := false
	rollbackSwapRename = func(oldPath, newPath string) error {
		if !injected && oldPath == guard && newPath == guard+".inx-rollback-aside" {
			injected = true
	...
	if got, err := os.ReadFile(target + ".inx-rollback-aside"); err != nil || string(got) != "new-desktop" {
...
func TestRollbackPendingUpdateStateRejectsDriftInsideRetainRename(t *testing.T) {
	...
	injected := false
	rollbackSwapRename = func(oldpath, newpath string) error {
		if !injected && oldpath == target && newpath == target+".inx-rollback-aside" {
			injected = true
...
func TestRollbackPendingAppBundleStateRejectsDriftInsideRetainRename(t *testing.T) {
	...
	injected := false
	rollbackSwapRename = func(oldpath, newpath string) error {
		if !injected && oldpath == app && strings.Contains(newpath, ".inx-failed-") {
			injected = true
...
func TestFileUpdateRollbackStageFailureLeavesInstallUntouched(t *testing.T) {
	...
	originalCopy := rollbackStageCopy
	rollbackStageCopy = func(src, dst string, mode os.FileMode) (string, error) {
		if strings.HasPrefix(dst, guard+".inx-rollback-stage-") {
...
func TestPrepareFileUpdateDoesNotOverwriteLegacyBackupName(t *testing.T) {
	...
	legacyBackup := filepath.Join(backupDir, "inx-desktop.previous")
...
func TestFileUpdateRollbackBypassesAndPreservesCrashedStage(t *testing.T) {
	...
	stage := target + ".inx-rollback-stage-crashed"
...
func TestRetainUpdateRollbackNodeRetriesNameCollision(t *testing.T) {
	target := filepath.Join(t.TempDir(), "Inx.app")
	...
	retained, err := retainUpdateRollbackNode(target, "inx-failed")
	if err != nil {
		t.Fatal(err)
	}
	if calls != 2 || !strings.Contains(retained, ".inx-failed-") {
...
func TestPublishClaimedFileUpdateMemberUsesCompareAndPublish(t *testing.T) {
	...
	aside := target + ".inx-update-aside-" + UpdateTransactionID(claimed)[:16]
...
func TestPublishClaimedFileUpdateMemberRestoresPreparedFileWhenStageSwapped(t *testing.T) {
	...
	fileUpdateAfterRetain = func(_, _ string) {
		stages, err := filepath.Glob(filepath.Join(dir, ".inx-desktop.inx-update-stage-*"))
		if err != nil {
			hookErr = err
			return
		}
	...
	if err == nil || !strings.Contains(err.Error(), "installed inx-desktop changed") {
	...
	rejected, err := filepath.Glob(target + ".inx-cleanup-*")
	if err != nil {
		t.Fatal(err)
	}
	if len(rejected) != 1 {
...
func TestFileUpdateRollbackCleansInstalledNodeBoundToTransaction(t *testing.T) {
	...
	if _, err := os.Stat(target + ".inx-rollback-aside"); !os.IsNotExist(err) {
...
func TestAppBundleRollbackPreservesLiveBundleWithoutReplacementOwnership(t *testing.T) {
	...
	failed, err := filepath.Glob(app + ".inx-failed-*")
	if err != nil || len(failed) != 1 {
		t.Fatalf("retained unbound bundle = %v, %v", failed, err)
	}
	if got, err := os.ReadFile(filepath.Join(failed[0], "Contents", "MacOS", "Inx")); err != nil || string(got) != "unbound-live" {
