...
func TestPlainInputWithStrongResearchSignalPreservesRefsWithoutStartingGoal(t *testing.T) {
	...
	if _, err := os.Stat(filepath.Join(root, ".inx", "autoresearch")); !os.IsNotExist(err) {
...
func TestPlainAutoResearchTaskPathDoesNotResumeGoal(t *testing.T) {
	...
	input := "继续 .inx/autoresearch/" + taskID + "/ 这个任务"
	c.Submit(input)
	waitForTurnDone(t, events)
...
func TestResearchGoalCreatesHostManagedAutoResearchTask(t *testing.T) {
	...
	for _, rel := range []string{
		...
		path := filepath.Join(root, ".inx", "autoresearch", state.AutoResearchTaskID, rel)
	...
	readJSONFileForTest(t, filepath.Join(root, ".inx", "autoresearch", state.AutoResearchTaskID, "state", "task_spec.json"), &spec)
...
func TestResearchGoalMissingExplicitTaskBlocksInsteadOfCreatingNewTask(t *testing.T) {
	...
	c.SetGoalWithResearchMode("resume .inx/autoresearch/missing-task/", GoalResearchOn)
