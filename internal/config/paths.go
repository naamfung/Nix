...
func inxHomeDir() string {
	if dir := cleanEnvDir("INX_HOME"); dir != "" {
		return dir
	}
	if runtimeGOOS == "windows" {
		if dir := osUserConfigDir(); dir != "" {
			return filepath.Join(dir, "inx")
		}
...
func legacyXDGConfigPaths() []string {
	...
	if dir := cleanEnvDir("XDG_CONFIG_HOME"); dir != "" {
		add(filepath.Join(dir, "inx", "config.toml"))
	}
	if home, err := osUserHomeDir(); err == nil && home != "" {
		add(filepath.Join(home, ".config", "inx", "config.toml"))
	}
	return paths
}

func userSupportDir() string {
	if dir := cleanEnvDir("INX_STATE_HOME"); dir != "" {
		return dir
	}
	return inxHomeDir()
}
...
func legacyOSSupportDir() string {
	...
	path := filepath.Join(dir, "inx")
	if current := inxHomeDir(); current != "" && samePath(path, current) {
		return ""
	}
	return path
}
...
func userCacheDir() string {
	if dir := cleanEnvDir("INX_CACHE_HOME"); dir != "" {
		return dir
	}
	if dir := cleanEnvDir("INX_HOME"); dir != "" {
...
// IsolatedHomeDir returns the INX_HOME directory when it has been
...
// real location instead of a hardcoded ~/.inx path.
func userConfigDisplayPath() string {
	p := userConfigPath()
	if p == "" {
		return "<os-config-dir>/inx/config.toml"
	}
	if home, err := osUserHomeDir(); err == nil && home != "" {
...
// UserConfigPath is the user-global config.toml. It lives under Inx home:
// INX_HOME/config.toml, then ~/.inx/config.toml on Unix-like systems,
// or %AppData%/inx/config.toml on Windows. If %AppData% is unavailable on
// Windows, it falls back to %USERPROFILE%/AppData/Roaming/inx/config.toml.
...
// from the current v1.8.1 Inx-home config path.
...
// InxManagedConfigPaths returns the Inx-owned user configuration
...
// TOML locations, and the legacy v0.x ~/.inx/config.json. Individual
// files, never directories — the Inx home also holds credentials (.env),
// global hooks (settings.json), skills, and session stores, and none of those
// may ride along on a config repair.
func InxManagedConfigPaths() []string {
	var out []string
...
// InxHomeDir is the current Inx home directory. It honors
// INX_HOME, then uses ~/.inx on macOS/Linux or %APPDATA%/inx on
// Windows, with a %USERPROFILE%/AppData/Roaming fallback when %APPDATA% is
// unavailable.
func InxHomeDir() string { return inxHomeDir() }

// RemoteStateDir is local state for the remote-SSH module (the managed
// known_hosts file, cached host metadata): <Inx home>/remote. Routed
...
// RemoteKnownHostsPath is the Inx-managed known_hosts file (OpenSSH
...
func WorkspaceLeaseDir() string {
	// Deliberately ignore INX_HOME/INX_CACHE_HOME here. Two app
	...
	return filepath.Join(dir, "inx", "workspace-leases")
}

// RepairMutationLockDir stores target-path repair locks in the OS-user cache.
// It deliberately ignores Inx home/cache overrides: isolated instances
// can still repair the same project inx.toml, so their locks must converge.
func RepairMutationLockDir() string {
	dir := osUserCacheDir()
	if strings.TrimSpace(dir) == "" {
		return ""
	}
	return filepath.Join(dir, "inx", "repair-mutation-locks")
}
...
// profile; other platforms keep using Inx state storage.
func DeliveryWorktreeDir() string {
	...
	if runtimeGOOS == "windows" {
		if dir := osUserCacheDir(); dir != "" {
			return filepath.Join(dir, "inx", "worktrees")
		}
		if home, err := osUserHomeDir(); err == nil && home != "" {
			return filepath.Join(home, "AppData", "Local", "inx", "worktrees")
		}
		return ""
	}
	dir := userSupportDir()
...
// UserCredentialsPath is the inx-owned global .env file under Inx
// home. It is the single source for provider credentials saved by Inx, so
// stale shell, Windows, project, or home env vars cannot silently override keys
// the user saved through setup or settings. "" when Inx home can't be
// resolved.
...
// Used by `inx --continue` / `--resume` to find the recent ones. Empty
...
// MemoryUserDir returns the inx user state root (…/inx), under which
// the user-global INX.md and the per-project auto-memory store live. Empty
...
// commands), in canonical-first order. .inx is ours; .agents / .agent /
...
// .inx/settings.json (see internal/hook).
var ConventionDirs = []string{".inx", ".agents", ".agent", ".claude"}

// conventionSubdirsAsc joins sub under each ConventionDir of base, in ascending
// priority (reverse of ConventionDirs) so the canonical .inx ends up the
...
// ~/.inx/commands), the Inx home commands dir, the legacy OS
// app-support dir if different, then the project's
// convention dirs (.claude/commands … .inx/commands). Scanning the .claude /
