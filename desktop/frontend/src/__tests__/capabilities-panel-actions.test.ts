...

{
  ...
  const meta: Meta = { label: "test", ready: true, eventChannel: "mcp-registry-channel", cwd: "/tmp/inx-test", workspaceRoot: "/tmp/inx-test" };
  const tabs: TabMeta[] = [{
    id: "tab-mcp-registry",
    scope: "project",
    workspaceRoot: "/tmp/inx-test",
    workspaceName: "inx-test",
...

{
  ...
  if (!rootEl) throw new Error("missing root");
  const root = createRoot(rootEl);
  const meta: Meta = { label: "test", ready: true, eventChannel: "test-channel", cwd: "/tmp/inx-test", workspaceRoot: "/tmp/inx-test" };
  const tabs: TabMeta[] = [{
    id: "tab-1",
    scope: "project",
    workspaceRoot: "/tmp/inx-test",
...

{
  ...
  if (!rootEl) throw new Error("missing root");
  const root = createRoot(rootEl);
  const meta: Meta = { label: "test", ready: true, eventChannel: "authorize-mcp-channel", cwd: "/tmp/inx-test", workspaceRoot: "/tmp/inx-test" };
  const tabs: TabMeta[] = [{
  ...
  ok(document.body.textContent?.includes("Install once and use automatically in every Inx project."), "global MCP explains its cross-project availability");
  ...
  ok(document.body.textContent?.includes("Current project · inx.toml"), "project MCP details show their configuration source");
...

{
  ...
  if (!rootEl) throw new Error("missing root");
  const root = createRoot(rootEl);
  const meta: Meta = { label: "test", ready: true, eventChannel: "managed-mcp-channel", cwd: "/tmp/inx-test", workspaceRoot: "/tmp/inx-test" };
  const tabs: TabMeta[] = [{
    id: "tab-managed-mcp",
    scope: "project",
    workspaceRoot: "/tmp/inx-test",
...

{
  ...
  if (!rootEl) throw new Error("missing root");
  const root = createRoot(rootEl);
  const meta: Meta = { label: "test", ready: true, eventChannel: "runtime-mcp-channel", cwd: "/tmp/inx-test", workspaceRoot: "/tmp/inx-test" };
  const tabs: TabMeta[] = [{
    id: "tab-runtime-mcp",
    scope: "project",
    workspaceRoot: "/tmp/inx-test",
...

{
  ...
  if (!rootEl) throw new Error("missing root");
  const root = createRoot(rootEl);
  const meta: Meta = { label: "test", ready: true, eventChannel: "mcp-editor-channel", cwd: "/tmp/inx-test", workspaceRoot: "/tmp/inx-test" };
  const tabs: TabMeta[] = [{
    id: "tab-mcp-editor",
    scope: "project",
    workspaceRoot: "/tmp/inx-test",
...

{
  ...
  if (!rootEl) throw new Error("missing root");
  const root = createRoot(rootEl);
  const meta: Meta = { label: "test", ready: true, eventChannel: "plugin-channel", cwd: "/tmp/inx-test", workspaceRoot: "/tmp/inx-test" };
  const tabs: TabMeta[] = [{
    id: "tab-plugin",
    scope: "project",
    workspaceRoot: "/tmp/inx-test",
