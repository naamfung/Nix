...
func TestGoSSHStreamSurfacesMissingRemoteCLI(t *testing.T) {
	...
	_, _ = stderr.Write([]byte("bash: inx: command not found\n"))
	stream := &goSSHStream{
...
func TestRemoteWorkbenchBinaryValidation(t *testing.T) {
	got, err := validateRemoteWorkbenchBinary("/home/dev user/.inx/remote/workbench/abc/inx")
	if err != nil || got == "" {
		t.Fatalf("valid path = %q err=%v", got, err)
	}
	for _, value := range []string{"relative/inx", "/tmp/../bin/inx", "/tmp/x\n/evil"} {
...
func TestWindowsWorkbenchDirectPreservesIdentityAndProxyJump(t *testing.T) {
	...
	entry := config.RemoteHostEntry{
		...
		IdentityFile: `C:\keys\inx key`, ProxyJump: "bastion-a,bastion-b",
	}
