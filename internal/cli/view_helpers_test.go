...
func TestRenderMemoryGroupsDocsAndStore(t *testing.T) {
	...
	got := renderMemory(width, &memory.Set{
		Docs:  []memory.Source{{Path: "/Users/me/project/INX.md", Scope: memory.ScopeProject}},
		Store: store,
		Index: store.Index(),
	})
	for _, want := range []string{
		"memory", "instructions", "precedence=1", "scope=project", "INX.md",
...
func TestRenderHooksUsesSharedVisualLanguage(t *testing.T) {
	...
	for _, want := range []string{"hooks (1 active)", "PreToolUse", "project", "…", ".inx/settings.json"} {
		if !strings.Contains(got, want) {
...
func TestRenderSkillPathsStaysWithinWidth(t *testing.T) {
	...
	got := renderSkillPaths(width, []skill.Root{{
		Dir:      "/Users/me/projects/really/deep/path/to/.inx/skills",
