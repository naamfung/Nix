...
// RemotePasswordCredentialEnvName returns the Inx-owned credential slot
...
// RemotePassphraseCredentialEnvName returns the Inx-owned credential slot
...
func remoteCredentialEnvName(hostID, kind string) string {
	...
	return fmt.Sprintf("INX_REMOTE_%X_%s", sum[:8], kind)
}

// IsGeneratedRemoteCredential reports whether key is a Inx-owned slot
...
// EditUserConfigWithCredentials updates config and its Inx-owned secret
