...
build:
	CGO_ENABLED=0 go build -ldflags "$(LDFLAGS)" -o bin/nix$(GOEXE) ./cmd/reasonix
	CGO_ENABLED=0 go build -ldflags "$(LDFLAGS)" -o bin/nix-plugin-example$(GOEXE) ./cmd/reasonix-plugin-example
...
cross:
	...
	@for p in darwin/amd64 darwin/arm64 linux/amd64 linux/arm64 windows/amd64 windows/arm64; do \
		...
		echo "build $$os/$$arch"; \
		CGO_ENABLED=0 GOOS=$$os GOARCH=$$arch go build -ldflags "$(LDFLAGS)" -o dist/nix-$$os-$$arch$$ext ./cmd/reasonix; \
	done
