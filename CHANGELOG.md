...
All notable changes to the Go line (Inx 1.0+) are recorded here. The legacy
`0.x` TypeScript history lives on the [`v1`](https://github.com/naamfung/inx/tree/v1)
branch.
...
- Added a **Remote SSH** module (VS Code Remote-SSH style): a user-global
  `[remote]` host config, `inx remote` CLI (add/list/remove/import/test/
  ...
  bootstraps a persistent `inx serve` on the remote host and tunnels its
...
- Added `inx serve --port-file/--token-file/--pid-file` so a supervised
...
- Fixed Desktop sessions incorrectly locking themselves during Goal + Delivery
  ...
  `INX_HOME` instead of the executable path. Switching saved sessions is
...
- Isolated the Windows desktop WebView2 shell from stale system proxies, so an
  ...
  WebView2 still does not reach DOM-ready within 15 seconds, Inx now shows
  the native window with a recovery prompt instead of appearing not to launch.
  Remote Markdown images are fetched by the backend with Inx's proxy
...
- **Go kernel**: a single static binary (CGO-free), cross-compiled for
  ...
  the native binary), Homebrew (`naamfung/inx` tap), and release archives;
...
- **Code intelligence via CodeGraph**: a tree-sitter symbol/call graph
  ...
  or API cost. Fetched into a local cache on first use (or `inx codegraph
...
- **Memory**: `INX.md` hierarchy + auto-memory, folded into the cache-stable
  prefix.
- **ACP** (`inx acp`) and an HTTP/SSE server frontend; desktop app (Wails).
