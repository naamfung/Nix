...
func startTUIDiagnostics(inxHome string) *tuiDiagnostics {
	d := &tuiDiagnostics{previous: slog.Default(), writer: io.Discard}
	if logDir := tuiDiagnosticLogDir(inxHome); logDir != "" {
...
func tuiDiagnosticLogDir(inxHome string) string {
	if strings.TrimSpace(inxHome) == "" {
		return ""
	}
	return filepath.Join(inxHome, "logs")
}
...
func (w *boundedDiagnosticWriter) Write(p []byte) (int, error) {
	...
	if n < total && !w.truncated {
		w.truncated = true
		_, _ = io.WriteString(w.dst, "\ninx: CLI TUI diagnostic log limit reached; further diagnostics omitted\n")
		w.remaining = 0
	}
	return total, nil
}
