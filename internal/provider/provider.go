...
// is opened by an older Inx binary that does not know Message.LocalOnly.
...
const (
	LocalOnlyToolName = "__inx_local_only__"
	LocalOnlyToolID   = "__inx_local_only__"
)
...
type ToolCall struct {
	...
	Added            int    `json:"added,omitempty"`
	Removed          int    `json:"removed,omitempty"`
	// Resolved* fields are Inx-local display metadata for stable proxy
...
func (e *AuthError) Error() string {
	...
	return fmt.Sprintf("authentication failed for provider %q (HTTP %d): %s is invalid or expired — update it (in .env or your environment) and retry, or run `inx setup`",
