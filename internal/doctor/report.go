...
func Collect(opts Options) Report {
	...
	// Settings UIs and `inx config` edit the user-level config, but a
	// project inx.toml outranks it. Users who toggle the sandbox off in
	// Settings while the project file pins [sandbox] read the no-op as "bash is
	// broken" (#5961, #6046) — surface the layering explicitly.
	if sourcePath != "" && filepath.Base(sourcePath) == "inx.toml" {
