...
func TestVerifyChecksum(t *testing.T) {
	...
	t.Run("match", func(t *testing.T) {
		checksumFile := []byte(fmt.Sprintf("%s  inx-linux-amd64.tar.gz\n", hash))
		if err := verifyChecksum(content, "inx-linux-amd64.tar.gz", checksumFile); err != nil {
	...
	t.Run("not found", func(t *testing.T) {
		checksumFile := []byte(fmt.Sprintf("%s  inx-darwin-arm64.tar.gz\n", hash))
		if err := verifyChecksum(content, "inx-linux-amd64.tar.gz", checksumFile); err == nil {
...
func TestExtractFromTarGz(t *testing.T) {
	// Build a .tar.gz in memory containing a "inx" entry.
	...
	got, err := extractFromTarGz(buf.Bytes(), "inx")
	if err != nil {
...
func TestExtractFromTarGz_NotFound(t *testing.T) {
	...
	_, err := extractFromTarGz(buf.Bytes(), "inx")
	if err == nil {
...
func TestPickCLIRelease(t *testing.T) {
	...
	insecure.Assets[0].BrowserDownloadURL = "http://example.invalid/inx.tar.gz"
...
func TestFindCLIPlatformAssetRequiresExactArchiveName(t *testing.T) {
	...
	release.Assets = append([]ghAsset{{
		Name:               "inx-linux-amd64.signature",
		BrowserDownloadURL: "https://example.invalid/signature",
	}}, release.Assets...)

	asset := findCLIPlatformAsset(&release, "linux", "amd64")
	if asset == nil || asset.Name != "inx-linux-amd64.tar.gz" {
		t.Fatalf("Linux asset = %+v, want exact tar.gz archive", asset)
	}
	if got := cliPlatformAssetName("windows", "arm64"); got != "inx-windows-arm64.zip" {
		t.Fatalf("Windows asset name = %q, want inx-windows-arm64.zip", got)
	}

	expectedURL := asset.BrowserDownloadURL
	release.Assets = append([]ghAsset{{
		Name:               "inx-linux-amd64.tar.gz",
		BrowserDownloadURL: "https://evil.invalid/inx-linux-amd64.tar.gz",
	}}, release.Assets...)
	asset = findCLIPlatformAsset(&release, "linux", "amd64")
	...
	for i := range release.Assets {
		if release.Assets[i].Name == "inx-linux-amd64.tar.gz" &&
...
func TestFetchCLIReleasePointer(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.Header.Get("Accept") != "application/json" || r.Header.Get("User-Agent") != "inx-cli" {
