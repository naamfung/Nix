...
// to drive Inx. Many tools integrated with the v1 (main-branch) agent over
...
// InxExtensionCapabilities advertises Inx-specific ACP extensions.
// ACP v1 reserves agentCapabilities._meta for vendor capability discovery.
type InxExtensionCapabilities struct {
...
// older map shape that Inx v1 clients used.
type MCPEnv map[string]string

// MCPHeaders accepts ACP's official HTTPHeader[] shape while still accepting
// the older map shape that Inx v1 clients used. The official spec
...
// SessionListResult is the first and only page of sessions Inx currently
...
// SessionSteerParams is the Inx ACP v1 extension for injecting user
...
const sessionSteerMethod = "_inx.io/session/steer"
...
// unsaved editor state. Line (1-based) and Limit page the content; Inx
