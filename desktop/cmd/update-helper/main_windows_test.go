...
func TestRunVersionedLayoutDoesNotReadOrClaimLegacyPending(t *testing.T) {
	...
	if err := installlayout.ActivateVersion(installlayout.ActivationRequest{
		...
		Members: []installlayout.Member{
			{Name: "inx-desktop.exe", Path: filepath.Join(seed, "inx-desktop.exe")},
			{Name: "inx-cli.exe", Path: filepath.Join(seed, "inx-cli.exe")},
			{Name: "inx-update-helper.exe", Path: filepath.Join(seed, "inx-update-helper.exe")},
		},
		RequiredNames: []string{"inx-desktop.exe", "inx-cli.exe", "inx-update-helper.exe"},
	}); err != nil {
		t.Fatal(err)
	}
	...
	runInstallerFn = func(_ string, staging string) error {
		for _, name := range []string{"inx-desktop.exe", "inx-cli.exe", "inx-update-helper.exe", "inx-launcher.exe"} {
	...
	if code := run([]string{
		...
		"--relaunch", filepath.Join(installDir, "inx-launcher.exe"),
		"--to-version", "v1.20.1",
...
func TestRunHoldsReleaseUnitLockAcrossInstallerHandoff(t *testing.T) {
	...
	if code := run([]string{
		...
		"--installer-sha256", testInstallerSHA256,
		"--install-dir", installDir,
		"--relaunch", filepath.Join(installDir, "inx-desktop.exe"),
	...
	wantClaim := "claim:" + strings.Join([]string{
		...
		filepath.Join(installDir, "inx-desktop.exe"),
...
func TestRunRelaunchesWhenLegacyPendingCannotBeClaimed(t *testing.T) {
	...
	startRelaunchFn = func(path, dir string) error {
		relaunched = path == filepath.Join(installDir, "inx-desktop.exe") && dir == installDir
		return nil
	}
	claimPendingFileUpdateFn = func(string, string, string, string, []string, time.Duration) (*repair.UpdateTransaction, func(), error) {
