...
// separate from the `inx setup` wizard (cli) so a GUI can apply one setting at a
...
// SetLanguage pins the CLI UI/model language; empty/auto clears the override so runtime detection falls back to INX_LANG / locale.
...
func (c *Config) SetCLIUpdateChannel(channel string) error {
	...
	case "", "stable", "preview", "canary", "beta", "next":
		c.CLI.UpdateChannel = ""
	default:
		return fmt.Errorf("CLI update channel %q is unsupported; Inx now uses the official release channel", channel)
	}
	return nil
}
...
// .mcp.json-only server would otherwise be serialized into inx.toml or the
...
// action cannot drift to another project's inx.toml or .mcp.json after the
...
func ClearPluginAuthenticationInSourceForRoot(root, name string) (PluginEntry, bool, string, error) {
	...
	if resolvedRoot != "." {
		projectTOML = filepath.Join(resolvedRoot, "inx.toml")
...
func pluginTOMLSourcePathForRoot(root, name string) string {
	...
	if resolved := resolveRoot(root); resolved != "." {
		projectTOML = filepath.Join(resolved, "inx.toml")
	}
...
// and legacy JSON receives a disable marker for older Inx versions.
...
// half-written inx.toml that fails to parse on next load. Parent directories
// are created as needed.
//
// For project configs (./inx.toml) the write is incremental: only sections
...
func SaveMinimalProjectReasoningLanguage(path, lang string) (string, error) {
	...
	body := fmt.Sprintf(`# Inx project configuration.
...
// IsUserConfigPath reports whether path is one of Inx's current or legacy
...
// (SourcePath), or to ./inx.toml when none exists yet — the conventional
...
// user's global config when root has no inx.toml. Existing project files
