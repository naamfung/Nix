...
func TestWindowsReleaseSignsPayloadBeforeRepackaging(t *testing.T) {
	...
	for _, want := range []string{
		...
		`INX_REQUIRE_PAYLOAD_MANIFEST`,
		`"-DARG_INX_SIGNED_UNINSTALLER=${uninstaller_path}"`,
...
func TestWindowsPackagerRejectsMissingOrPartialRequiredPayloadManifest(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.manifest {
				if err := os.WriteFile(filepath.Join(payload, "inx-payload.json"), []byte("{}"), 0o600); err != nil {
					t.Fatal(err)
				}
			}
			if tc.signature {
				if err := os.WriteFile(filepath.Join(payload, "inx-payload.json.minisig"), []byte("sig"), 0o600); err != nil {
					t.Fatal(err)
				}
			}
			cmd := exec.Command("bash", "../scripts/package-windows-desktop.sh", "amd64", payload)
			cmd.Env = append(os.Environ(), "INX_REQUIRE_PAYLOAD_MANIFEST=1")
...
func TestSignPathConfigurationsCoverExactWindowsPayload(t *testing.T) {
	expected := map[string]bool{
		"inx-desktop.exe":       true,
		"inx-guard.exe":         true,
		"inx-launcher.exe":      true,
		"inx-update-helper.exe": true,
		"inx-cli.exe":           true,
		"inx-uninstall.exe":     true,
	}
