//go:build windows && inx_remote_integration
...
import (
	...
	remotebroker "inx/internal/remote/broker"
	"inx/internal/remote/protocol"
	workbenchclient "inx/internal/remote/workbench/client"
)

const (
	remoteWorkbenchIntegrationGateEnv        = "INX_REMOTE_WORKBENCH_INTEGRATION"
	remoteWorkbenchIntegrationHostEnv        = "INX_REMOTE_WORKBENCH_HOST"
	remoteWorkbenchIntegrationPortEnv        = "INX_REMOTE_WORKBENCH_PORT"
	remoteWorkbenchIntegrationUserEnv        = "INX_REMOTE_WORKBENCH_USER"
	remoteWorkbenchIntegrationIdentityEnv    = "INX_REMOTE_WORKBENCH_IDENTITY_FILE"
	remoteWorkbenchIntegrationWorkspaceEnv   = "INX_REMOTE_WORKBENCH_WORKSPACE"
	remoteWorkbenchIntegrationVersionEnv     = "INX_REMOTE_WORKBENCH_EXPECTED_VERSION"
	remoteWorkbenchIntegrationExpectedSHAEnv = "INX_REMOTE_WORKBENCH_EXPECTED_SHA"
	remoteWorkbenchIntegrationFingerprintEnv = "INX_REMOTE_WORKBENCH_FINGERPRINT"
	remoteWorkbenchIntegrationMarker         = "inx-remote-workbench-physical-tool-result"
)
...
func TestRemoteWorkbenchWindowsToLinuxPhysicalAcceptance(t *testing.T) {
	if os.Getenv(remoteWorkbenchIntegrationGateEnv) != "1" {
		t.Skip("set INX_REMOTE_WORKBENCH_INTEGRATION=1 to run the physical Windows to Linux acceptance test")
	}
	...
	newClient := func(generation uint64) (*workbenchclient.Client, *windowsWorkbenchSSHFactory) {
		t.Helper()
		factory, factoryErr := newWindowsWorkbenchSSHFactoryForBinary(entry, "inx", func(context.Context, RemoteAskPassPrompt) (RemoteAskPassAnswer, error) {
