...
// TestLoadDotEnvReadsGlobalCredentials proves `inx setup`'s target — the
// inx-owned credentials file under Inx home — is loaded from any
...
func TestDotEnvFileFilteredPreservesProjectScopeRules(t *testing.T) {
	...
	if _, ok := got["INX_HOME"]; ok {
		t.Fatalf("INX_HOME should be filtered: %+v", got)
	}
	if _, ok := got["HOME"]; ok {
...
func TestLoadForRootFiltersProjectDotEnvControlVars(t *testing.T) {
	...
	t.Setenv("XDG_CONFIG_HOME", filepath.Join(home, ".config"))
	t.Setenv("AppData", filepath.Join(home, "AppData"))
	t.Setenv("INX_STATE_HOME", "")
	os.Unsetenv("INX_STATE_HOME")

	wantCred := UserCredentialsPath()
	if wantCred == "" {
		t.Skip("user credentials path unavailable")
	}
	if err := os.WriteFile(filepath.Join(project, ".env"), []byte("INX_STATE_HOME="+redirect+"\nPLUGIN_TOKEN=project-token\n"), 0o600); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(filepath.Join(project, ".mcp.json"), []byte(`{
  "mcpServers": {
    "svc": {
      ...
      "headers": {
        ...
        "State": "${INX_STATE_HOME:-default-state}"
      }
    }
  }
}`), 0o644); err != nil {
	...
	if got := os.Getenv("INX_STATE_HOME"); got != "" {
...
func TestCredentialResolverCachesGlobalFirstLookups(t *testing.T) {
	...
	stubStoredCredentialValueForTest(t, func(got string) (string, CredentialSource, bool) {
		...
		return "from_credentials", CredentialSource{Kind: CredentialSourceCredentials, Label: "Inx credentials"}, true
	})
...
func TestResolveCredentialSourceShowsCredentialsShadowingProjectEnv(t *testing.T) {
	...
	if !got.Set || got.Source.Kind != CredentialSourceCredentials {
		t.Fatalf("source = %+v set=%v, want Inx credentials", got.Source, got.Set)
	}
...
func TestUserCredentialsPathIgnoresInxStateHome(t *testing.T) {
	home := t.TempDir()
	state := t.TempDir()
	t.Setenv("HOME", home)
	t.Setenv("USERPROFILE", home)
	...
	want := filepath.Join(home, "inx-home", ".env")
	...
	if strings.HasPrefix(UserCredentialsPath(), state) {
		t.Fatalf("credentials path must not live under INX_STATE_HOME: %q", UserCredentialsPath())
	}
}
...
func TestProjectConfigCannotOverrideCredentialStoreMode(t *testing.T) {
	...
	project := t.TempDir()
	t.Setenv("INX_CREDENTIALS_STORE", "")
	os.Unsetenv("INX_CREDENTIALS_STORE")
	...
	if err := os.WriteFile(filepath.Join(project, "inx.toml"), []byte(`credentials_store = "keyring"`), 0o644); err != nil {
		t.Fatal(err)
	}
...
// TestLoadDotEnvGlobalCredentialsOverrideEnv confirms Inx-owned global
