...
// permissionMeta carries Inx-owned structured data that an ACP supervisor
	...
	if a.Tool == "bash" && strings.TrimSpace(s.cwd) != "" {
		...
		if json.Unmarshal(a.RawInput, &input) == nil &&
			...
			if cwdErr == nil && commandErr == nil && exact && len(command.Argv) > 0 {
				inx["commandSchemaVersion"] = 1
				inx["argv"] = command.Argv
				inx["cwd"] = cwd
			}
		}
	}
	return map[string]any{"inx.io": inx}
}
