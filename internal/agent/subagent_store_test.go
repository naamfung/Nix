...
func TestSubagentStoreCleanupStaleRunningSkipsForeignLiveParent(t *testing.T) {
	...
	cmd.Env = append(os.Environ(),
		"INX_SUBAGENT_LEASE_HELPER=1",
		"INX_SUBAGENT_LEASE_PATH="+parentPath,
	)
...
func TestSubagentStoreForeignLeaseHelper(t *testing.T) {
	if os.Getenv("INX_SUBAGENT_LEASE_HELPER") != "1" {
		return
	}
	lease, err := TryAcquireSessionLease(os.Getenv("INX_SUBAGENT_LEASE_PATH"))
	if err != nil {
