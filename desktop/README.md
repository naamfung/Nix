# Inx Desktop (Wails shell)

A native desktop window around the Inx Go kernel. The same
...
`desktop/` is its own Go module (`module inx/desktop`, `replace inx =>
...
`inx/` so it can still import the `inx/internal/*` kernel.
...
```sh
cd desktop
wails build          # → build/bin/Inx(.app/.exe)
```
...
`crash.inx.io` desktop release gateway) and shows an update banner when a
...
- **Linux Debian/Ubuntu (`.deb`)** — download the signed `.deb`, request administrator
  ...
  `sudo apt install ./Inx-linux-amd64.deb` (no uninstall required). After
...
- **macOS** — open `Inx-darwin-universal.dmg` and drag Inx into
  ...
  ```sh
  xattr -dr com.apple.quarantine /Applications/Inx.app
  ```
...
```sh
minisign -Vm Inx-darwin-arm64.zip \
...
- **Linux / WebKitGTK** is the one real pain point — rendering varies by distro &
  ...
  - **Wayland + NVIDIA**: On KDE Plasma Wayland with NVIDIA GPUs, WebKitGTK can
    ...
    Inx automatically sets `__NV_DISABLE_EXPLICIT_SYNC=1` when it detects
...
- **Windows / WebView2** — `Theme: SystemDefault` follows the OS light/dark
  ...
  `INX_DESKTOP_DISABLE_WEBVIEW2_GPU=1` or `0` to force the fallback on or
  ...
  using Inx's own proxy configuration. Remote Markdown images are fetched
...
The desktop app sends one anonymous ping per launch to `crash.inx.io`:
