...
Running `inx` without a subcommand starts the interactive terminal UI. Use
`inx setup` first when no provider is configured.
...
```sh
inx upgrade                  # install the latest official release
inx upgrade --check          # report the target without installing
inx upgrade --force          # reinstall the current official release
```
...
time Inx saves the configuration. The `inx update` alias behaves the
same way.

## Configure providers

```sh
inx setup                    # manage the user-global config
inx setup --local            # manage ./inx.toml
inx setup /path/to/config.toml
```

In an interactive terminal, `inx setup` is a staged provider manager. It
...
are stored in the shared Inx home `.env`, even with `--local`. When a
...
```sh
inx config currency             # show the saved and resolved currency
inx config currency auto        # follow the resolved locale
...
be overridden by project `inx.toml`; `--local` is therefore not supported.
...
```sh
inx -p "list the risky changes" --output-format text
inx -p "summarize the diff" --output-format json
inx run "run the tests" --output-format stream-json
```
...
```sh
inx run --events-jsonl "run the focused tests"
```
...
store. The two options cannot be combined. Without either option, Inx
...
remain stable for the same session and Inx user-state directory, while a
...
the Inx state directory if automation depends on existing machine IDs.
...
inx run -y "apply the requested changes"
inx -p "run the focused tests" --allowed-tools "Bash(go test ./...)"
...
`inx run --auto ...` (or `-y`). The alias cannot be combined with an
...
In non-interactive runs (`inx run` / `-p`) there is no prompt to answer, so
...
```sh
inx --add-dir ../shared
inx -p "update both projects" \
  --add-dir ../frontend \
  --add-dir ../backend
```

Relative paths resolve from the workspace root and must already exist as
directories. Inx resolves symlinks, removes duplicates, and extends the
...
macOS and the terminal's configured shortcut elsewhere). While Inx owns the
