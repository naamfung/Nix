...
func TestRemoteUnsupportedCapabilitiesNeverFallBackToLocal(t *testing.T) {
	...
	checks := []struct {
		...
		{"save doc", func() error { _, err := app.SaveDoc("INX.md", "local-only"); return err }},
		{"save doc for tab", func() error { _, err := app.SaveDocForTab("local-tab", "INX.md", "local-only"); return err }},
