...
test_root="$(mktemp -d "${TMPDIR:-/tmp}/inx-release-workflow-test.XXXXXX")"
cleanup() {
	case "$test_root" in
	*/inx-release-workflow-test.*) rm -rf -- "$test_root" ;;
...
grep -Fq 'cmp -s /tmp/inx-desktop-canary-latest.json /tmp/inx-desktop-preview-latest.json' \
	"$repo_root/.github/workflows/release-desktop.yml"
...
jq -n \
	...
	--argjson names "$manifest_assets" '
	{
		...
		prerelease: false,
		html_url: ("https://github.com/" + $repo + "/releases/tag/" + $tag),
		release_notes_url: ("https://inx.io/changelog/" + $tag + "/"),
		assets: [
			$names[] as $name |
			{
				name: $name,
...
jq '.release_notes_url = "https://inx.io/changelog/v9.9.9/"' \
...
jq \
	...
	.prerelease = true |
	.html_url = ("https://github.com/" + $repo + "/releases/tag/" + $tag) |
	.release_notes_url = ("https://inx.io/changelog/" + $notes_tag + "/") |
	.assets |= map(
		.browser_download_url =
			("https://github.com/" + $repo + "/releases/download/" + $tag + "/" + .name)
	)
...
printf '{"version":"v1.2.3","release_notes_url":"https://inx.io/changelog/v1.2.3/","marker":"candidate"}\n' \
	>"$recovery_candidate_directory/latest.json"
printf '{"version":"v1.2.3","release_notes_url":"https://inx.io/changelog/v1.2.3/","marker":"existing"}\n' \
...
jq -n '{
	version: "v1.2.3",
	release_notes_url: "https://inx.io/changelog/v1.2.3/",
...
jq -n \
	--arg url "https://dl.inx.io/desktop-v1.2.3/payload.zip" \
	--arg sha "$manifest_asset_sha" \
	--argjson size "$manifest_asset_size" \
	--arg native_url "https://dl.inx.io/desktop-v1.2.3/payload.deb" \
...
write_desktop_manifest() {
	...
	jq -n --arg version "$version" --arg notes_version "$notes_version" \
		...
		{
			version: $version,
			download_page: "https://inx.io/?download=desktop#start",
			release_notes_url: ("https://inx.io/changelog/" + $notes_version + "/"),
			platforms: {
				"darwin-arm64": asset("Inx-darwin-arm64.zip"),
				"darwin-amd64": asset("Inx-darwin-amd64.zip"),
				"windows-amd64": asset("Inx-windows-amd64-installer.exe"),
				"windows-arm64": asset("Inx-windows-arm64-installer.exe"),
				"linux-amd64": asset("Inx-linux-amd64.tar.gz")
			},
			native_packages: {
				"linux-amd64": asset("Inx-linux-amd64.deb")
			},
			downloads: {
				"Inx-darwin-universal.dmg": asset("Inx-darwin-universal.dmg"),
				"Inx-windows-amd64.zip": asset("Inx-windows-amd64.zip")
			}
		}
	' >"$output"
}
...
desktop_stable_base="https://dl.inx.io/desktop-${desktop_stable_version}/"
...
desktop_preview_base="https://dl.inx.io/desktop-${desktop_preview_version}/"
...
desktop_rc_base="https://dl.inx.io/desktop-${desktop_rc_version}/"
...
jq '.downloads["Inx-darwin-universal.dmg"].sha256 = ("A" * 64)' \
...
jq 'del(.downloads["Inx-windows-amd64.zip"])' \
...
write_desktop_manifest "$desktop_preview_version" "https://dl.inx.io/desktop-preview/" \
...
bash "$desktop_validator" legacy-preview "$desktop_preview_version" \
	"https://dl.inx.io/desktop-preview/" "$test_root/desktop-legacy-preview.json"
...
expect_invalid_desktop_manifest "partial downloads in a legacy Preview manifest" legacy-preview \
	"$desktop_preview_version" "https://dl.inx.io/desktop-preview/" \
...
(
	...
	ACTUAL_CALLER_WORKFLOW_REF='example/inx/.github/workflows/release-stable.yml@refs/tags/v1.2.3' \
		EXPECTED_CALLER_WORKFLOW_REF='example/inx/.github/workflows/release-stable.yml@refs/tags/v1.2.3' \
	...
	ACTUAL_CALLER_WORKFLOW_REF='example/inx/.github/workflows/release-preview.yml@refs/heads/main-v2' \
		EXPECTED_CALLER_WORKFLOW_REF='example/inx/.github/workflows/release-preview.yml@refs/heads/main-v2' \
	...
	ACTUAL_CALLER_WORKFLOW_REF='example/inx/.github/workflows/release-stable.yml@refs/heads/main-v2' \
		EXPECTED_CALLER_WORKFLOW_REF='example/inx/.github/workflows/release-stable.yml@refs/heads/main-v2' \
		CALLER_EVENT_NAME=workflow_dispatch CALLER_REF=refs/heads/main-v2 CALLER_REF_PROTECTED=true \
	...
	RELEASE_TAG=v1.2.3 APPROVED_SHA="$approved_sha" VERIFY_RELEASE_CHECKOUT=false \
		"$repo_root/scripts/verify-release-tag.sh"
	if ACTUAL_CALLER_WORKFLOW_REF='example/inx/.github/workflows/release-stable.yml@refs/heads/main-v2' \
		EXPECTED_CALLER_WORKFLOW_REF='example/inx/.github/workflows/release-stable.yml@refs/heads/main-v2' \
		CALLER_EVENT_NAME=workflow_dispatch CALLER_REF=refs/heads/main-v2 CALLER_REF_PROTECTED=true \
	...
	if ACTUAL_CALLER_WORKFLOW_REF='example/inx/.github/workflows/release-stable.yml@refs/heads/topic' \
		EXPECTED_CALLER_WORKFLOW_REF='example/inx/.github/workflows/release-stable.yml@refs/heads/topic' \
