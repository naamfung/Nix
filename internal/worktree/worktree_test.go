...
func TestCreateManagedWorktreeFromRepositoryFolder(t *testing.T) {
	...
	if !strings.HasPrefix(result.Branch, "inx/delivery-") {
	...
	if _, err := os.Stat(filepath.Join(result.WorktreeRoot, ".inx-source")); !os.IsNotExist(err) {
		t.Fatalf("worktree must not contain a Inx marker, stat err = %v", err)
	}
...
func TestCreatePreservesSelectedRepositorySubdirectory(t *testing.T) {
	...
	cmd = exec.Command("git", "-C", repo, "-c", "user.name=Inx Test", "-c", "user.email=inx@example.invalid", "commit", "-m", "subdir")
	if out, err := cmd.CombinedOutput(); err != nil {
...
func TestInspectRejectsUncommittedSelectedSubdirectoryWithoutGitMutation(t *testing.T) {
	...
	before, _, err := runGit(context.Background(), repo, "for-each-ref", "--format=%(refname)", "refs/heads/inx/delivery-")
	if err != nil {
		t.Fatal(err)
	}
	...
	after, _, err := runGit(context.Background(), repo, "for-each-ref", "--format=%(refname)", "refs/heads/inx/delivery-")
	if err != nil {
		t.Fatal(err)
	}
	if after != before {
...
func TestCreateSupportsPathsWithSpaces(t *testing.T) {
	...
	git := func(args ...string) {
		cmd := exec.Command("git", append([]string{"-C", repo}, args...)...)
		cmd.Env = append(os.Environ(), "GIT_AUTHOR_NAME=Inx Test", "GIT_AUTHOR_EMAIL=inx@example.invalid", "GIT_COMMITTER_NAME=Inx Test", "GIT_COMMITTER_EMAIL=inx@example.invalid")
		if out, err := cmd.CombinedOutput(); err != nil {
