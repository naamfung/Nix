...
func newIdleTimeoutManager(ctx context.Context, initialTimeout, extendTimeout time.Duration) (context.Context, *idleTimeoutManager) {
	...
	m := &idleTimeoutManager{
		...
		deadline: now.Add(initialTimeout),
	}
	...
	go func() {
		...
		for {
			...
			case <-m.resetCh:
				...
				// extendTimeout from its current value (monotonic, never
				// shortened), then re-arm the timer for what remains.
				m.deadline = m.deadline.Add(extendTimeout)
				if !m.timer.Stop() {
					select {
					case <-m.timer.C:
					default:
...
func (b bash) runForeground(ctx context.Context, p bashParams, sh sandbox.Shell, argv []string, wrapped bool, cmdEnv []string) (string, error) {
	// Use idle timeout mechanism: initial 10 minutes, then extend by 2 minutes on activity.
	runCtx, idleMgr := newIdleTimeoutManager(ctx, idleTimeoutInitial, idleTimeoutExtend)
	defer func() {
		// Cancel the idle timeout context to clean up the manager's goroutine
		idleMgr.cancel(context.Canceled)
	}()
	...
	// Wrap w to trigger idle reset on write
	w = &idleResetWriter{w: w, resetFn: idleMgr.reset}
	...
	if errors.Is(context.Cause(runCtx), errBashIdleTimeout) {
		return out, fmt.Errorf("command timed out (> %s)", idleTimeoutInitial)
	}
	if err != nil {
