...
Inx 提供 CLI 与桌面端 **设置 → 诊断** 共用的只读能力诊断模型，覆盖 Skills、
...
| 目标 | 命令 / 入口 |
| --- | --- |
| 检查当前工作区的 skills / hooks / MCP / 插件 | `inx doctor capabilities` |
| 机器可读报告（CI / 报障） | `inx doctor capabilities --json` |
| 指定项目根目录 | `inx doctor capabilities --root /path/to/project` |
| 真实探测 MCP 启动（会启动第三方服务器） | `inx doctor capabilities --live --timeout 5s` |
| 让 Agent 按手册排障 | 会话中 `/inx-guide`，或自然语言描述症状 |
| GUI 健康视图 | 桌面端 **设置 → 诊断** |
...
```bash
inx doctor                  # 环境 / provider / 沙箱快照
inx doctor session <id>     # 支持用会话包
inx doctor redact-sessions  # 脱敏会话中的密钥
```
...
然后到 **设置 → 技能**，或直接改 `.inx/skills` / `.inx/commands` 下的文件。
...
项目 Hooks 会从 `.inx/settings.json` 自动加载。若没有触发，请确认当前工作区，
保存后重启 Inx。`match` 是**锚定**正则：`file` **不会**匹配 `read_file`。
...
`inx-guide` 会覆盖内置版；也可用
`[skills].disabled_skills = ["inx-guide"]` 隐藏。
...
既有 `inx doctor` / `doctor session` / `doctor redact-sessions` 的 JSON
schema **不会**混入新字段。
...
| 需求 | 改用 |
| --- | --- |
| Provider 密钥、代理、沙箱 OS 支持 | `inx doctor` |
| 给支持用的完整会话包 | `inx doctor session <id>` |
| 单个插件包 | `inx plugin doctor <name>` |
| 会话内 MCP 列表 | `/mcp` |

## 缓存影响

内置 `inx-guide` 仅在 system prompt 的 Skill 索引中增加 **一行稳定索引**；
