...
Use `inx plugin` when installing or managing plugin packages from a
terminal. Plugin packages are installed globally under the Inx home
directory.
...
- A local directory that contains `inx-plugin.json`,
  `.codex-plugin/plugin.json`, or `.claude-plugin/plugin.json`.

Preview the install plan without writing files:

```bash
inx plugin install git:github.com/obra/superpowers --dry-run
```

Install a plugin after reviewing the plan:

```bash
inx plugin install git:github.com/obra/superpowers --yes
```

Install with an explicit name or replace an installed plugin with the same name:

```bash
inx plugin install git:github.com/obra/superpowers --name superpowers --replace --yes
```

Use a local directory in developer mode:

```bash
inx plugin install /path/to/plugin --link --replace --yes
```
...
- `--link` links a local plugin directory instead of copying it into Inx's
  plugin storage. Moving or deleting that directory breaks the linked plugin.

Running `inx plugin install <source>` without `--dry-run` or `--yes`
...
```bash
inx plugin disable superpowers
inx plugin enable superpowers
```

Remove a plugin:

```bash
inx plugin remove superpowers --yes
```
...
Inx loads its capabilities into normal interactive sessions:
...
- **MCP servers** join the normal MCP/tool flow. Ask for the task you want done;
  Inx can call the plugin's tools when they are relevant.

After installing, enabling, disabling, or updating a plugin from a separate
terminal while a session is already running, start a new `inx` session or
...
- **Developer mode: link source folder** appears for **Local folder** installs.
  It links the selected directory instead of copying it into Inx's plugin
...
Inx plugins can declare `inx-plugin.json` at the plugin root:
...
Relative paths are resolved inside the plugin root. Inx does not run
...
Inx construct; it does not by itself guarantee every runtime decision an
...
such as Superpowers and Claude-style skill packs, Inx maps:

- `skills` to Inx skill roots. A Claude manifest that declares no
...
- `commands/` (and `.claude/commands/`) to Inx custom slash commands: each
  ...
  details report that conflict. Native `inx-plugin.json` manifests can
...
- `agents/*.md` to manually invoked, plugin-owned subagent profiles. Claude
  model aliases inherit the active Inx model; inline `tools` lists map to
  Inx tool names, including wildcard MCP names such as `mcp__*__search`.
  Agents use `/<plugin>:agent:<name>`, so an upstream agent and skill may share
  the same name without shadowing one another.
- `hooks/session-start-codex` to the Inx `SessionStart` hook when present.
- A plugin-root `CLAUDE.md` file to a built-in `SessionStart` context hook. The
  file is read directly by Inx, without spawning a shell command.
- `.claude/settings.json` and `hooks/hooks.json` command hooks to Inx hook
  ...
  translated between Inx's own tool names and Claude's (`bash` ↔
  `Bash`, `write_file` ↔ `Write`, ...), so a matcher like `"Bash"` fires
  correctly; every Inx subagent-spawning tool (`task`, `read_only_task`,
  ...
  includes Claude's required `prompt` and `description`; Inx supplies a
  stable operation label when its tool call omitted the optional description.
  `tool_input` keys that
  Inx names differently from Claude are renamed too — `path` becomes
  ...
  task content. `NotebookEdit` also supplies `new_source` from Inx's
  ...
  interrupted}` shape (Inx combines both streams into `stdout`; the
  ...
  expands `${CLAUDE_PLUGIN_ROOT}` and `${INX_PLUGIN_ROOT}` (plus their
  ...
  explicit Bash hooks. `inx plugin doctor <name>` and
  `inx doctor capabilities` report a missing required shell before the
  ...
  `Stop`/`SubagentStop` hook (which cannot block the turn in Inx), or a
  matcher that covers one of three inputs Inx cannot losslessly express:
  `WebFetch.prompt`, `NotebookEdit.cell_id` for a Inx `cell_number` call,
  or `TaskOutput.task_id` when Inx `wait` covers multiple/all jobs. Each
...
Unsupported Claude hook item types are skipped with a warning. Inx does not
