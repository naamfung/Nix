...
// by substring (file names embed the key, e.g. Inx-darwin-arm64.zip), so the
...
// (inx.key) and the public key (inx.pub) into dir. The password comes
...
func genKey(dir string) error {
	...
	keyPath := filepath.Join(dir, "inx.key")
	pubPath := filepath.Join(dir, "inx.pub")
...
func signFiles(files []string) error {
	...
	for _, f := range files {
		...
		sig := minisign.SignWithComments(priv, data,
			"file:"+filepath.Base(f), "Inx desktop release")
		out := f + ".minisig"
...
func genManifest(dir, version, tag string, notesVersions ...string) error {
	...
	m := update.Manifest{
		Version:         version,
		DownloadPage:    "https://inx.io/?download=desktop#start",
		ReleaseNotesURL: "https://inx.io/changelog/" + notesVersion + "/",
