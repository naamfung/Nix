...
const PACK_STYLE_ID = "inx-theme-pack-overlay";
const TOKEN_KEYS = [
  "bg",
  "bgSoft",
  "bgElev",
...
// that Wails serves through /__inx_theme_asset/. Only exact, internally
...
export function isSafeBackgroundURL(url: string): boolean {
  const u = url.trim();
  if (!u) return false;
  if (u.startsWith("/__inx_theme_asset/")) return true;
