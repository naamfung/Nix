...
func TestSessionSidecarLayout(t *testing.T) {
	const p = "/home/u/.inx/sessions/abc.jsonl"
	cases := []struct {
		name string
		got  string
		want string
		...
		{"goal-state", SessionGoalState(p), "/home/u/.inx/sessions/abc.goal-state.json"},
		{"event-log", SessionEventLog(p), "/home/u/.inx/sessions/abc.events.jsonl"},
		{"event-log-damaged", SessionEventLogDamaged(p), "/home/u/.inx/sessions/abc.events.jsonl.damaged"},
		{"event-index", SessionEventIndex(p), "/home/u/.inx/sessions/abc.event-index.json"},
		{"conflict-log", SessionConflictLog(p), "/home/u/.inx/sessions/abc.conflicts.jsonl"},
		{"lock", SessionLockFile(p), p + ".lock"},
		{"lease-lock", SessionLeaseLock(p), p + ".lease.lock"},
		{"lease-info", SessionLeaseInfo(p), p + ".lease.json"},
		{"checkpoint", SessionCheckpointDir(p), "/home/u/.inx/sessions/abc.ckpt"},
		{"jobs", SessionJobsDir(p), "/home/u/.inx/sessions/abc.jobs"},
		{"cleanup-pending", SessionCleanupPending(p), "/home/u/.inx/sessions/abc.cleanup-pending.json"},
	}
	for _, c := range cases {
		if c.got != c.want {
...
func TestSessionSidecarFiles(t *testing.T) {
	...
	want := []string{
		p + ".meta",
		"/home/u/.inx/sessions/abc.goal-state.json",
		"/home/u/.inx/sessions/abc.events.jsonl",
		"/home/u/.inx/sessions/abc.events.jsonl.damaged",
		"/home/u/.inx/sessions/abc.event-index.json",
		"/home/u/.inx/sessions/abc.conflicts.jsonl",
		"/home/u/.inx/sessions/abc.recovery.json",
	}
	if len(got) != len(want) {
