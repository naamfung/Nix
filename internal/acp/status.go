...
type InxSchemaCapability struct {
...
// coupling the ACP adapter to Inx configuration internals.
...
type InxFinalReadiness struct {
	ReadyForReview bool     `json:"readyForReview"`
	Summary        string   `json:"summary"`
	Risks          []string `json:"risks"`
}

type InxUsage struct {
...
type InxStatusUsage struct {
	Turn       InxUsage `json:"turn"`
	Cumulative InxUsage `json:"cumulative"`
}
...
func (a usageAccumulator) wire() InxUsage {
	usage := InxUsage{
...
type statusTelemetry struct {
	...
	state          string
	phase          string
	turnOutcome    InxTurnOutcome
	finalReadiness InxFinalReadiness
...
func newStatusTelemetry() *statusTelemetry {
	return &statusTelemetry{
		state:       "idle",
		phase:       "idle",
		turnOutcome: InxTurnOutcome{Kind: "none"},
		finalReadiness: InxFinalReadiness{
			Risks: []string{},
		},
	}
...
func (t *statusTelemetry) beginTurn() {
	t.mutate(func(t *statusTelemetry) {
		t.state = "running"
		t.phase = "starting"
		t.turnOutcome = InxTurnOutcome{Kind: "none"}
		t.finalReadiness = InxFinalReadiness{Risks: []string{}}
...
func (t *statusTelemetry) finishTurn(runErr error, cancelled bool, goalStatus, summary string) string {
	...
	t.mutate(func(t *statusTelemetry) {
		...
		case goalStatus == control.GoalStatusBlocked:
			t.phase = "paused"
			t.turnOutcome = InxTurnOutcome{Kind: "paused", Reason: "goal blocked"}
			eventName = "pause"
		default:
			...
			case errors.As(runErr, &readinessErr):
				...
				t.turnOutcome = InxTurnOutcome{Kind: "paused", Reason: clipStatusText(readinessErr.Error(), 2_048)}
			...
			case errors.As(runErr, &recoveryPause):
				...
				t.turnOutcome = InxTurnOutcome{Kind: "paused", Reason: clipStatusText(recoveryPause.Error(), 2_048)}
				eventName = "pause"
			case runErr != nil:
				t.phase = "error"
				t.turnOutcome = InxTurnOutcome{Kind: "error", Reason: clipStatusText(runErr.Error(), 2_048)}
				t.goalOverride = "failed"
				eventName = "error"
			default:
				t.phase = "paused"
				t.turnOutcome = InxTurnOutcome{Kind: "paused", Reason: "goal is not complete"}
				eventName = "pause"
			}
		}
	})
...
func restoreStatusTelemetry(saved *persistedStatusTelemetry) *statusTelemetry {
	...
	t.finalReadiness = InxFinalReadiness{
	...
	if interrupted {
		t.sequence++
		t.phase = "recovery_paused"
		t.turnOutcome = InxTurnOutcome{Kind: "paused", Reason: "previous turn interrupted"}
...
func (s *service) publishStatus(sess *acpSession, eventName string) {
	...
	_ = s.conn.Notify(sessionStatusUpdateMethod, InxStatusUpdate{
		SchemaVersion: inxStatusSchemaVersion,
...
func (s *acpSession) statusSnapshot() InxSessionStatus {
	s.mu.Lock()
	id := s.id
	ctrl := s.ctrl
	model := s.model
