...

{
  ...
  const dom = installDom();
  mockApp({
    SavePastedFile: async () => ".inx/attachments/notes.txt",
  });
  ...
  ok(
    calls.submit[0]?.includes("@.inx/attachments/notes.txt") === true,
...

{
  ...
  mockApp({
    AttachDropped: async () => ({
      kind: "attachment",
      path: ".inx/attachments/report.pdf",
    }),
  });
  const { root } = await renderComposer();
  if (!droppedCallback) throw new Error("native file drop handler did not register");
...

{
  ...
  mockApp({
    AttachDropped: async () => ({
      kind: "workspace",
      path: "__inx_external_folder/mock/Folder-With-Spaces",
      isDir: true,
  ...
  eq(calls.submit.join(","), "inspect @__inx_external_folder/mock/Folder-With-Spaces/", "external folder submit text uses the session ref token");
...
{
  const externalToken = "__inx_external_folder/mock/Folder-With-Spaces/src/outside.txt";
    ...
    root.render(
      <LocaleProvider>
        <UserMessage
          ...
          submitText={"/inx-develop Compare these commands\n/other-command"}
        />
      </LocaleProvider>,
    );
    await flushTimers();
  });
  ok(document.querySelector(".invocation-display--message")?.textContent?.includes("Inx Develop") === true, "history recovery ignores slash-prefixed lines inside the task body");
...

{
  ...
  mockApp({
    ...
    SavePastedFile: async () => {
      savedFiles += 1;
      return ".inx/attachments/notes.txt";
    },
  });
  const { root, rerender } = await renderComposer();
  const textarea = document.querySelector("textarea") as HTMLTextAreaElement | null;
