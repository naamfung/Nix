...
const (
	macBundleID         = "com.wails.inx-desktop"
	macUpdateHandoffArg = "--inx-mac-update-handoff"
	macHandoffReadyFD   = 3
...
func applyMac(zipPath, targetVersion string) error {
	...
	backupApp := currentApp + ".inx-update-backup"
...
func parseMacUpdateHandoffArgs(args []string) (macUpdateHandoffConfig, error) {
	fs := flag.NewFlagSet("inx-mac-update-handoff", flag.ContinueOnError)
...
func runMacUpdateHandoff(cfg macUpdateHandoffConfig) int {
	...
	rollback := func() error {
		...
		if publishedReplacement {
			var retainErr error
			failedApp, retainErr = retainMacHandoffNode(oldApp, "inx-update-failed")
			if retainErr != nil {
		...
		if err := repair.VerifyAppBundleUpdateHandoffOriginal(claimed); err != nil {
			rejected, retainErr := retainMacHandoffNode(oldApp, "inx-update-rejected")
			if retainErr != nil {
	...
	installRoot, err := os.MkdirTemp(filepath.Dir(oldApp), ".inx-update-install-*")
	if err != nil {
...
func completeMacHandoffHandshake(cfg macUpdateHandoffConfig) error {
	...
	ready := os.NewFile(uintptr(cfg.ReadyFD), "inx-update-ready")
	proceed := os.NewFile(uintptr(cfg.ProceedFD), "inx-update-proceed")
...
func findMacApp(root string) (string, error) {
	direct := filepath.Join(root, "Inx.app")
