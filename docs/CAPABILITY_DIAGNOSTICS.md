...
Inx ships a read-only capability diagnostics model shared by the CLI and
desktop **Settings → Diagnostics**. It reports Skills, Commands, Hooks, plugin
packages, MCP servers, and instruction docs (`AGENTS.md` / `INX.md` /
`CLAUDE.md`).
...
| Goal | What to run |
| --- | --- |
| Check this workspace’s skills / hooks / MCP / plugins | `inx doctor capabilities` |
| Machine-readable report (CI / support) | `inx doctor capabilities --json` |
| Another project root | `inx doctor capabilities --root /path/to/project` |
| Probe MCP startup for real (starts third-party servers) | `inx doctor capabilities --live --timeout 5s` |
| Ask the agent to walk through config / fix guidance | `/inx-guide` in chat, or ask naturally |
...
```bash
inx doctor                  # env / providers / sandbox snapshot
inx doctor session <id>     # support session bundle
inx doctor redact-sessions  # redact secrets in session files
```
...
```bash
inx doctor capabilities --json | jq '.skills.entries, .commands.entries, .issues'
```
...
Then open **Settings → Skills** (or fix the file under `.inx/skills` /
`.inx/commands`).

### 2. “Project hooks never fire”

```bash
inx doctor capabilities | sed -n '/Hooks/,/Plugins/p'
```

Project hooks load automatically from `.inx/settings.json`. If they do not
fire, confirm the active workspace and restart Inx after saving. Matchers
...
1. Static first (no side effects):
   ...
   ```bash
   inx doctor capabilities --json | jq '.mcp.servers, .issues[] | select(.subsystem=="mcp")'
   ```

2. Only if you accept starting third-party servers:

   ```bash
   inx doctor capabilities --live --timeout 10s --json
   ```
...
global skills named `inx-guide` override the builtin; you can also hide it
with `[skills].disabled_skills = ["inx-guide"]`.

## CLI reference

```bash
inx doctor capabilities [--root PATH] [--json] [--live] [--timeout 5s]
```
...
inx doctor capabilities --live --timeout 15s --json 2>live-warn.txt
```

Existing `inx doctor`, `doctor session`, and `doctor redact-sessions`
...
```json
{
  ...
  "subsystem": "skills",
  "name": "demo",
  "source": "<workspace>/.inx/skills/demo/SKILL.md",
  "message": "...",
  "remediation": "...",
  "settings_tab": "skills"
}
...
| Need | Use instead |
| --- | --- |
| Provider keys, proxy, sandbox OS support | `inx doctor` |
| Full session transcript for support | `inx doctor session <id>` |
| One plugin package only | `inx plugin doctor <name>` |
| Interactive MCP list in a chat session | `/mcp` |

## Cache impact

Adding the built-in `inx-guide` skill appends **one stable index line** to
