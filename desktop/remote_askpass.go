...
const (
	...
	remoteAskPassModeEnv     = "INX_REMOTE_ASKPASS"
	remoteAskPassEndpointEnv = "INX_REMOTE_ASKPASS_ENDPOINT"
	remoteAskPassKeyEnv      = "INX_REMOTE_ASKPASS_KEY"
	remoteAskPassDeadlineEnv = "INX_REMOTE_ASKPASS_DEADLINE_UNIX_MS"
)
...
func remoteAskPassRequestMAC(key, token []byte, prompt string) []byte {
	...
	_, _ = mac.Write([]byte("inx-askpass-v1\x00"))
	_, _ = mac.Write(token)
