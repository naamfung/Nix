...
func init() {
	...
	for i := range contracts {
		codes[i] = string(contracts[i].InxCode)
	}
	enumTypes[reflect.TypeOf(InxErrorCode(""))] = codes
}
...
func allowedCLICommand(command string) bool {
	switch command {
	case "inx remote install", "inx remote start", "inx remote stop",
		"inx remote restart", "inx remote status", "inx remote doctor",
		"inx remote logs", "inx remote uninstall", "inx setup":
		return true
	default:
		return false
	}
