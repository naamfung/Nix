...
import (
	...
	wbtransport "inx/internal/remote/workbench/transport"
)
...
// StartConfigured preserves explicit Inx host fields while still letting
...
// from the saved Inx Host while retaining the injection-safe direct target
...
func (f *RemoteSSHTransportFactory) start(ctx context.Context, args []string) (*RemoteSSHTransport, error) {
	...
	if f.AskPass != nil {
		...
		overrides = append(overrides, "DISPLAY=inx-askpass")
...
func classifyRemoteSSHExit(stderr []byte, bootstrapStarted bool, contextErr error) *RemoteSSHConnectionFailure {
	...
	if !bootstrapStarted && containsAnyRemoteSSHDiagnostic(lower,
		"inx: command not found", "inx: not found",
		"/inx: not found", "'inx' is not recognized", "exec: inx: not found",
		"unknown remote subcommand \"attach-workspace\"") {
		return &RemoteSSHConnectionFailure{
			Code: RemoteSSHCLINotFound, Stage: RemoteSSHStageBootstrap,
			Retryable: false, Message: "The Inx CLI was not found on the remote Host.",
		}
	}
	if containsAnyRemoteSSHDiagnostic(lower,
