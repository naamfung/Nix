...
func TestApplyLocalSkillRootRegistersPath(t *testing.T) {
	...
	cfg := config.LoadForEdit(filepath.Join(project, "inx.toml"))
...
func TestApplyLocalCodexPluginPackage(t *testing.T) {
  ...
  "skills": "./skills/"
	...
	statePath := filepath.Join(home, ".inx", "plugin-packages.json")
	...
	if _, err := os.Stat(filepath.Join(home, ".inx", "plugins", "superpowers", ".codex-plugin", "plugin.json")); err != nil {
...
func TestApplyLocalClaudePluginPackage(t *testing.T) {
  ...
  "skills": "./.claude/skills/"
	...
	if _, err := os.Stat(filepath.Join(home, ".inx", "plugins", "ui-ux-pro-max", ".claude-plugin", "plugin.json")); err != nil {
		t.Fatalf("installed plugin missing: %v", err)
	}
}
...
func TestApplyCopiedPluginPreservesExecutableHookCommand(t *testing.T) {
  ...
  "hooks":{"UserPromptSubmit":[{"hooks":[{"type":"command","command":"${CLAUDE_PLUGIN_ROOT}/bin/hook","args":["--hook"]}]}]}
	...
	installed := filepath.Join(home, ".inx", "plugins", "executable-plugin", "bin", "hook")
...
func TestPlanClaudeCompatibilityReportsAgentsHooksAndMCP(t *testing.T) {
	...
	a := planned.Actions[0]
	// A Stop hook is imported best-effort, but Inx's Stop hook is
...
func TestApplyLocalSkillFileCopiesToProject(t *testing.T) {
	...
	target := filepath.Join(project, ".inx", "skills", "beta", "SKILL.md")
...
func TestApplyLocalSkillFileDoesNotShadowFlatCompatInstall(t *testing.T) {
	project := t.TempDir()
	home := t.TempDir()
	existing := filepath.Join(project, ".inx", "skills", "beta.md")
...
func TestApplyLocalSKILLFileCopiesSiblingResources(t *testing.T) {
	...
	target := filepath.Join(project, ".inx", "skills", "frontend-design", "SKILL.md")
	if resp.Actions[0].CanonicalPath != target {
		t.Fatalf("canonicalPath = %q, want %q", resp.Actions[0].CanonicalPath, target)
	}
	if _, err := os.Stat(filepath.Join(project, ".inx", "skills", "frontend-design", "references", "style.md")); err != nil {
		t.Fatalf("reference file should be copied with SKILL.md source: %v", err)
	}
	if _, err := os.Stat(filepath.Join(project, ".inx", "skills", "frontend-design", "scripts", "lint.sh")); err != nil {
...
func TestApplyLocalSkillLinkMode(t *testing.T) {
	...
	target := filepath.Join(project, ".inx", "skills", "gamma", "SKILL.md")
...
func TestPlanProjectMCPJSONDefaultsProject(t *testing.T) {
  ...
  "mcpServers": {
	...
	wantPath := filepath.Join(project, "inx.toml")
...
func TestApplyRemoteMCPURLDefaultsGlobal(t *testing.T) {
	...
	projectCfg := config.LoadForEdit(filepath.Join(project, "inx.toml"))
...
func TestApplyMCPRejectsDuplicateByDefault(t *testing.T) {
	...
	if err := cfg.SaveTo(filepath.Join(project, "inx.toml")); err != nil {
		t.Fatal(err)
	}
...
func TestApplyMCPReplaceOverwritesExisting(t *testing.T) {
	...
	reloaded := config.LoadForEdit(filepath.Join(project, "inx.toml"))
...
func TestGlobalSkillInstallRootUsesInxHome(t *testing.T) {
	home := t.TempDir()
	...
	want := filepath.Join(inxHome, skill.SkillsDirname)
	if root != want {
...
func TestUninstallRemovesSkillByName(t *testing.T) {
	project := t.TempDir()
	home := t.TempDir()
	target := filepath.Join(project, ".inx", "skills", "doomed.md")
	writeFile(t, target, "---\nname: doomed\ndescription: Doomed\n---\nbody")
...
func TestUninstallWithoutScopePrefersProjectSkill(t *testing.T) {
	project := t.TempDir()
	home := t.TempDir()
	projectTarget := filepath.Join(project, ".inx", "skills", "dupe.md")
	globalTarget := filepath.Join(home, ".inx", "skills", "dupe.md")
...
func TestPlanIDIncludesActionDetails(t *testing.T) {
	...
	a := action{Kind: "mcp", Action: "install_mcp_server", Name: "same", URL: "https://mcp.one.example/mcp", Transport: "http", ConfigPath: "/repo/inx.toml"}
	b := a
...
func TestGitHubClaudeMarketplacePlansAndAppliesRelativePlugins(t *testing.T) {
  ...
  "plugins": [
	...
	for _, name := range []string{"alpha-legal", "beta-legal"} {
		if _, ok, err := pluginpkg.FindInstalled(filepath.Join(home, ".inx"), name); err != nil || !ok {
...
func TestGitHubPluginApplyRefusesUnpinnableDrift(t *testing.T) {
	...
	if _, ok, _ := pluginpkg.FindInstalled(filepath.Join(home, ".inx"), "pwf"); ok {
...
func TestCopyMaterializesInRootSymlinkedCommands(t *testing.T) {
	...
	installedRoot := filepath.Join(home, ".inx", "plugins", "aliases")
...
func TestCopyRefusesUnmaterializableSymlinkCommands(t *testing.T) {
	...
	if _, err := os.Stat(filepath.Join(home, ".inx", "plugins", "escapes")); !os.IsNotExist(err) {
		t.Fatal("failed install must not leave the copied tree behind")
	}
	if _, ok, _ := pluginpkg.FindInstalled(filepath.Join(home, ".inx"), "escapes"); ok {
...
func TestFailedReplaceKeepsExistingPluginInstall(t *testing.T) {
	...
	installedRoot := filepath.Join(home, ".inx", "plugins", "pwf")
	...
	if p, ok, _ := pluginpkg.FindInstalled(filepath.Join(home, ".inx"), "pwf"); !ok || !p.Enabled {
...
func TestBackupPathCannotCollideWithSiblingPlugin(t *testing.T) {
	...
	siblingRoot := filepath.Join(home, ".inx", "plugins", "foo.pre-replace")
	...
	if _, ok, _ := pluginpkg.FindInstalled(filepath.Join(home, ".inx"), "foo.pre-replace"); !ok {
		t.Fatal("sibling plugin must stay registered")
	}
	pkg, _, err := pluginpkg.ParseDir(filepath.Join(home, ".inx", "plugins", "foo"))
	if err != nil {
