...
Inx implements Agent Client Protocol (ACP) v1 as an NDJSON JSON-RPC 2.0
...
```sh
inx acp
inx acp --model deepseek-pro
inx acp --profile delivery
```
...
Standard output is reserved for ACP messages. Inx sends diagnostics to
standard error, so hosts must not merge the two streams. Run `inx setup`
beforehand when no provider is configured; the initialize response also
advertises a terminal authentication method that launches `inx setup`.

## Initialize and negotiate capabilities

Clients should call `initialize` before opening a session. Inx advertises
...
`terminal`, Inx routes eligible file operations through the editor's
...
locally inside the Inx process.

## Session lifecycle

Each ACP session owns an independent Inx controller, workspace root, model,
...
Inx accepts stdio, Streamable HTTP, and legacy SSE servers. ACP's official `[{"name":"...","value":"..."}]`
shape is supported for stdio `env` and HTTP `headers`; the older object-map
shape remains accepted for compatibility.

## Session controls

Inx exposes independent controls instead of combining unrelated choices in
one mode selector:
...
audio are not advertised. During a turn, Inx may send:
...
Hosts should keep the `session/prompt` request open until Inx returns its
stop reason, while continuing to process requests and notifications in both
directions.

## Mid-turn steering extension

Inx exposes mid-turn guidance as an ACP v1 vendor extension. It is not a
...
```text
agentCapabilities._meta["inx.io"].sessionSteer.method
```
...
A successful `{}` result means the active turn accepted the guidance. Inx
...
not Inx's internal steer marker.
...
| Surface | Older or non-Inx clients | Conclusion |
| --- | --- | --- |
...
1. Launch `inx acp` with separate stdin, stdout, and stderr streams.
...
4. Process agent-to-client filesystem, terminal, and permission requests while
   a prompt is running.
5. Show steer UI only when the Inx capability is advertised and a prompt
   is active.
