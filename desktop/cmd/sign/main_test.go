...
func TestSignFiles(t *testing.T) {
	...
	dir := t.TempDir()
	artifact := filepath.Join(dir, "Inx-linux-amd64.tar.gz")
...
func TestGenManifest(t *testing.T) {
	...
	names := []string{
		...
		"Inx-windows-amd64.zip", // portable download, not the updater channel
		"Inx-windows-arm64-installer.exe",
		"Inx-windows-arm64.zip", // portable download, not the updater channel
	...
	t.Setenv("GITHUB_REPOSITORY", "esengine/inx")
	...
	if m.DownloadPage != "https://inx.io/?download=desktop#start" {
		t.Fatalf("download_page = %q, want official install page", m.DownloadPage)
	}
	if m.ReleaseNotesURL != "https://inx.io/changelog/v1.2.0/" {
	...
	if !strings.HasSuffix(arm.URL, "/Inx-windows-arm64-installer.exe") {
	...
	if !strings.HasSuffix(lin.URL, "/Inx-linux-amd64.tar.gz") {
	...
	if !strings.HasSuffix(deb.URL, "/Inx-linux-amd64.deb") {
	...
	for _, name := range []string{"Inx-darwin-universal.dmg", "Inx-windows-amd64.zip"} {
...
func TestGenManifestCanReuseStableNotesForStandaloneRC(t *testing.T) {
	dir := t.TempDir()
	if err := os.WriteFile(filepath.Join(dir, "Inx-linux-amd64.tar.gz"), []byte("rc"), 0o644); err != nil {
		t.Fatal(err)
	}
	...
	if m.ReleaseNotesURL != "https://inx.io/changelog/v1.3.0/" {
