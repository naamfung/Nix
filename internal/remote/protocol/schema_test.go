...
func TestCanonicalSchemaDeterministicAndHashExact(t *testing.T) {
	...
	text := string(third)
	for _, forbidden := range []string{"InitializeParams", "/home/taibai/DeepSeek-Inx", "generatedAt"} {
...
func TestSchemaContainsRegistryErrorsEventsAndFrozenLimits(t *testing.T) {
	...
	inxCode := findProperty(t, document.ErrorData, "inxCode")
	if len(inxCode.Schema.Enum) != 51 {
		t.Fatalf("InxErrorCode enum = %d", len(inxCode.Schema.Enum))
	}
