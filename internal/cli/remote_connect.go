...
func remoteConnectCLI(args []string, version string) int {
	...
	rest := fs.Args()
	if len(rest) != 1 {
		fmt.Fprintln(os.Stderr, "usage: inx remote connect <name> [flags]")
		return 2
	}
	name := rest[0]
...
func remoteServeCLI(args []string, version string) int {
	if len(args) < 2 {
		fmt.Fprintln(os.Stderr, "usage: inx remote serve start|stop|status|logs <name> [--workspace PATH] [-n N]")
		return 2
	}
	action := args[0]
...
func remoteForwardCLI(args []string) int {
	if len(args) < 2 {
		fmt.Fprintln(os.Stderr, "usage: inx remote forward add <host> (-L|-R) <spec> | rm <host> <name> | ls <host>")
		return 2
	}
	switch args[0] {
	case "ls":
...
func remoteForwardAdd(args []string) int {
	if len(args) != 3 {
		fmt.Fprintln(os.Stderr, "usage: inx remote forward add <host> (-L|-R) <spec>")
		return 2
	}
	host := args[0]
...
func remoteForwardRm(args []string) int {
	if len(args) != 2 {
		fmt.Fprintln(os.Stderr, "usage: inx remote forward rm <host> <bind>")
		return 2
	}
...
func remoteFSCLI(args []string) int {
	if len(args) < 2 {
		fmt.Fprintln(os.Stderr, "usage: inx remote fs ls <name>:<path> | get <name>:<remote> [local] | put <local> <name>:<remote>")
		return 2
	}
	switch args[0] {
	case "ls":
...
func remoteFSLs(operand string) int {
	...
	if !ok {
		fmt.Fprintln(os.Stderr, "usage: inx remote fs ls <name>:<path>")
		return 2
	}
...
func remoteFSGet(args []string) int {
	if len(args) < 1 || len(args) > 2 {
		fmt.Fprintln(os.Stderr, "usage: inx remote fs get <name>:<remote> [local]")
		return 2
	}
...
func remoteFSPut(args []string) int {
	if len(args) != 2 {
		fmt.Fprintln(os.Stderr, "usage: inx remote fs put <local> <name>:<remote>")
		return 2
	}
	localPath := args[0]
