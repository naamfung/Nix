...
func TestLockUserConfigEditsSerializesRMW(t *testing.T) {
	...
	path := UserConfigPath()
	if path == "" {
		t.Fatal("UserConfigPath is empty with INX_HOME set")
	}
...
func assertUserConfigLockSerializesAcrossProcesses(t *testing.T, firstHome, secondHome, firstTmp, secondTmp string) {
	...
	startHelper := func(mode, processHome, processTmp, started, acquired, release string) (*exec.Cmd, *bytes.Buffer) {
		...
		cmd.Env = testEnvWithOverrides(map[string]string{
			...
			"INX_CONFIG_LOCK_STARTED":  started,
			"INX_CONFIG_LOCK_ACQUIRED": acquired,
			"INX_CONFIG_LOCK_RELEASE":  release,
		})
...
func TestLockUserConfigEditsHelperProcess(t *testing.T) {
	...
	started := os.Getenv("INX_CONFIG_LOCK_STARTED")
	acquired := os.Getenv("INX_CONFIG_LOCK_ACQUIRED")
	release := os.Getenv("INX_CONFIG_LOCK_RELEASE")
	...
	switch os.Getenv("INX_CONFIG_LOCK_MODE") {
	case "bot":
		cfg.Bot.Connections = []BotConnectionConfig{{
			ID:       "cross-process",
			Provider: "qq",
...
func TestConfigEditLockCanonicalizesAliasesAndIgnoresCacheOverrides(t *testing.T) {
	...
	link := filepath.Join(dir, "inx.toml")
	...
	t.Setenv("INX_CACHE_HOME", filepath.Join(dir, "cache-a"))
	first, err := configFileEditLockPath(link)
	if err != nil {
		t.Fatal(err)
	}
	t.Setenv("INX_CACHE_HOME", filepath.Join(dir, "cache-b"))
	...
	t.Setenv("INX_HOME", filepath.Join(dir, "inx-home"))
