...
jq -e \
	...
	(type == "object") and
	(.version == $version) and
	(.download_page == "https://inx.io/?download=desktop#start") and
	(if $legacy
		then (.release_notes_url == null or .release_notes_url == ("https://inx.io/changelog/" + $notes_version + "/"))
		else (.release_notes_url == ("https://inx.io/changelog/" + $notes_version + "/"))
	end) and
	...
	(.platforms["darwin-arm64"] | valid_asset("Inx-darwin-arm64.zip")) and
	(.platforms["darwin-amd64"] | valid_asset("Inx-darwin-amd64.zip")) and
	(.platforms["windows-amd64"] | valid_asset("Inx-windows-amd64-installer.exe")) and
	(.platforms["windows-arm64"] | valid_asset("Inx-windows-arm64-installer.exe")) and
	(.platforms["linux-amd64"] | valid_asset("Inx-linux-amd64.tar.gz")) and
	(.native_packages | exact_keys(["linux-amd64"])) and
	(.native_packages["linux-amd64"] | valid_asset("Inx-linux-amd64.deb")) and
	(if $legacy and (.downloads == null)
		then true
			...
			(.downloads["Inx-darwin-universal.dmg"] | valid_asset("Inx-darwin-universal.dmg")) and
			(.downloads["Inx-windows-amd64.zip"] | valid_asset("Inx-windows-amd64.zip"))
	end)
' "$manifest" >/dev/null
