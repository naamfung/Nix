...
function desktopManifest(version: string, base?: string) {
  const releaseBase = base ?? `https://dl.inx.io/desktop-${version}/`;
...
describe("desktop Preview release gateway", () => {
  ...
  it("serves a complete Preview manifest from the canonical pointer first", async () => {
    ...
    expect(response.headers.get("x-inx-release-source")).toBe("r2-preview");
    expect(fetchMock).toHaveBeenCalledTimes(1);
    expect(fetchMock.mock.calls[0]?.[0]).toBe("https://dl.inx.io/preview/latest.json");
  });

  it("keeps serving the signed rolling Preview manifest during pointer migration", async () => {
    const legacy = desktopManifest(
      "v1.18.0-preview.62",
      "https://dl.inx.io/desktop-preview/",
    );
  ...
  it("continues to the compatibility pointer after an invalid 200 response", async () => {
    ...
    expect(response.status).toBe(200);
    expect(response.headers.get("x-inx-release-source")).toBe("r2-canary-compat");
    expect(fetchMock.mock.calls.map((call) => call[0])).toEqual([
      "https://dl.inx.io/preview/latest.json",
      "https://dl.inx.io/canary/latest.json",
    ]);
  });
  ...
  it("rejects hostile URLs and incomplete Desktop manifests", async () => {
    const cases: Array<[string, (manifest: ReturnType<typeof desktopManifest>) => void]> = [
      ["malicious host", (manifest) => {
        const url = "https://evil.invalid/desktop-v1.2.0-preview.7/Inx-darwin-arm64.zip";
        Object.assign(manifest.platforms["darwin-arm64"], { url, sig: `${url}.minisig` });
      }],
      ["userinfo", (manifest) => {
        const url = "https://dl.inx.io@evil.invalid/desktop-v1.2.0-preview.7/Inx-darwin-arm64.zip";
        Object.assign(manifest.platforms["darwin-arm64"], { url, sig: `${url}.minisig` });
      }],
      ["http", (manifest) => {
        const url = "http://dl.inx.io/desktop-v1.2.0-preview.7/Inx-darwin-arm64.zip";
        Object.assign(manifest.platforms["darwin-arm64"], { url, sig: `${url}.minisig` });
      }],
      ["wrong channel path", (manifest) => {
        const url = "https://dl.inx.io/preview/Inx-darwin-arm64.zip";
        Object.assign(manifest.platforms["darwin-arm64"], { url, sig: `${url}.minisig` });
      }],
      ["wrong filename", (manifest) => {
        const url = "https://dl.inx.io/desktop-v1.2.0-preview.7/Inx-darwin-amd64.zip";
        Object.assign(manifest.platforms["darwin-arm64"], { url, sig: `${url}.minisig` });
      }],
      ...
      ["missing website download", (manifest) => {
        delete (manifest.downloads as Partial<typeof manifest.downloads>)["Inx-darwin-universal.dmg"];
      }],
      ["invalid website download", (manifest) => {
        manifest.downloads["Inx-windows-amd64.zip"].size = 0;
      }],
      ["bad SHA", (manifest) => {
...
describe("desktop Stable GitHub fallback", () => {
  it("accepts the exact versioned R2 asset directory", async () => {
    ...
    expect(response.status).toBe(200);
    expect(response.headers.get("x-inx-release-source")).toBe("r2-stable");
    expect(fetchMock).toHaveBeenCalledTimes(1);
  });
...
describe("release gateway HTTP method contract", () => {
  ...
  it("answers CORS preflight without loading an upstream release", async () => {
    ...
    const response = await worker.fetch(
      new Request("https://crash.inx.io/v1/cli/releases/stable/latest.json", {
        method: "OPTIONS",
      }),
      env,
    );
  ...
  it("serves HEAD with GET status and headers but no body", async () => {
    ...
    const response = await worker.fetch(
      new Request("https://crash.inx.io/v1/desktop/releases/preview/latest.json", {
        method: "HEAD",
      }),
      env,
    );
  ...
  it("returns a CORS-aware 405 for unsupported release methods", async () => {
    ...
    const response = await worker.fetch(
      new Request("https://crash.inx.io/v1/cli/releases/preview/latest.json", {
        method: "POST",
      }),
      env,
    );
...
describe("CLI public release gateway", () => {
  ...
  it("serves a complete strict Preview pointer from R2", async () => {
    ...
    expect(response.headers.get("access-control-allow-origin")).toBe("*");
    expect(response.headers.get("x-inx-release-source")).toBe("r2-cli-preview");
    expect(fetchMock.mock.calls[0]?.[0]).toBe("https://dl.inx.io/cli/preview/latest.json");
  });
  ...
  it("falls back after an invalid 200 and strictly filters GitHub releases", async () => {
    ...
    expect(response.headers.get("x-inx-release-source")).toBe("github-cli-releases");
