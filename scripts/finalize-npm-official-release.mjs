...
const packages = [
  "inx",
  "@inx/cli-darwin-arm64", "@inx/cli-darwin-x64",
  "@inx/cli-linux-arm64", "@inx/cli-linux-x64",
  "@inx/cli-win32-arm64", "@inx/cli-win32-x64",
];

function metadata(name) {
  const output = execFileSync(
    "npm",
    ["view", `${name}@${version}`, "name", "version", "gitHead", "inxCandidateSha", "--json"],
    { encoding: "utf8" },
  );
...
for (const name of packages) {
  ...
  if (expectedSha && actual.inxCandidateSha !== expectedSha) {
    throw new Error(`${name}@${version} inxCandidateSha ${actual.inxCandidateSha || "<missing>"} does not match ${expectedSha}`);
  }
}
