...
build:
	CGO_ENABLED=0 go build -ldflags "$(LDFLAGS)" -o bin/inx$(GOEXE) ./cmd/inx
	CGO_ENABLED=0 go build -ldflags "$(LDFLAGS)" -o bin/inx-plugin-example$(GOEXE) ./cmd/inx-plugin-example
...
cross:
	...
	@for p in darwin/amd64 darwin/arm64 linux/amd64 linux/arm64 windows/amd64 windows/arm64; do \
		...
		echo "build $$os/$$arch"; \
		CGO_ENABLED=0 GOOS=$$os GOARCH=$$arch go build -ldflags "$(LDFLAGS)" -o dist/nix-$$os-$$arch$$ext ./cmd/inx; \
	done
