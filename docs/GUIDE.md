...
Resolution order: **flag > `./inx.toml` > the user config file >
built-in defaults**. Starting with **Inx v1.8.1**, the user config lives at
`~/.inx/config.toml` on macOS/Linux and
`%AppData%\inx\config.toml` on Windows; see
[Configuration paths](./CONFIG_PATHS.md) for migration and related data paths.
Fields marked user/global only are not overridden by `./inx.toml`.
...
provider keys or Inx control variables. See
...
# language    = "zh"               # ui language; empty = auto-detect from $LANG / $INX_LANG
...
Most day-to-day settings belong in `config.toml` or the global Inx `.env`
described above. The variables below are process-level advanced switches; set
them before launching Inx. Project `.env` files are not a runtime source for
Inx control variables.
...
Inx never uploads prompts, answers, reasoning, tool names/arguments/output,
...
`<Inx home>/cli-crash-reports`. Inx keeps at most 10 files with owner-only
...
```bash
inx report                 # preview newest; prompt before sending on a TTY
inx report list            # list local reports
inx report show [ID]       # preview without sending
inx report send [ID]       # explicit send; delete locally only after success
inx report delete [ID]     # delete without sending
```

Piped or redirected `inx report` calls only preview and never prompt or
...
`inx serve` starts the same local engine behind a browser UI. Use it when
you want a desktop-style surface without installing the desktop app, when running
Inx on a remote development box through a tunnel, or when you want a
...
```bash
inx serve --auth token
inx serve --addr 0.0.0.0:8787 --auth token
inx serve --auth password --password 'temporary-password'
```
...
```bash
inx serve --hash-password --password 'strong-password'

# <Inx home>/config.toml
[serve]
...
`inx acp` exposes Inx as an ACP v1 stdio agent for editors and other
...
MCP servers, permission requests, and the Inx mid-turn steering extension.

## Remote SSH

The remote module runs Inx on a remote host and reaches it over your own
SSH connection — VS Code Remote-SSH style. It bootstraps a persistent headless
`inx serve` on the remote host, forwards a local loopback port to it, and
...
`[secrets]`, a project `inx.toml` cannot inject or override remote hosts —
a cloned repo can never steer where Inx opens SSH connections. Credentials
follow the provider idiom: the host names an env var (`passphrase_env`,
`password_env`) whose value lives in Inx's global `.env`; key material
...
```bash
inx remote add gpu-box dev@203.0.113.7 --workspace '~/projects/app'
inx remote import --all              # import aliases; ssh -G resolves Include/Match rules when connecting
inx remote test gpu-box              # dial + auth + host-key confirmation
inx remote connect gpu-box --open    # bootstrap serve, tunnel, open the URL
inx remote serve status gpu-box
inx remote fs ls gpu-box:'~/projects/app'
```
...
plus a Inx-managed `~/.inx/remote/known_hosts`. A first-seen key
...
Remote-side state lives under the remote host's `~/.inx/remote/`:
...
each other. Inx can prefill editable custom-provider entries for Kimi CN,
Kimi Global,
...
usually needs only the provider API key: the key value is stored in Inx home
...
Inx preserves complete assistant messages across turns, sends output limits
...
chat path. In this mode Inx previews and sends chat requests to:
...
example `https://gateway.example.com/v1/chat/completions`. Inx then sends
...
| --- | --- | --- |
| `api_key_env` | The environment-variable name used for this provider's API key. Desktop-saved key values are stored in Inx home `.env` under this name; the TOML config stores only the name. | Change it when several providers need distinct keys, or leave it blank for a service that does not require an API key. |
...
| Extra request body | A JSON object merged into the top-level chat request body. | Use only for provider-specific flags such as `{"enable_thinking": true}`. Inx still owns core fields such as `model`, `messages`, `tools`, `stream`, and `thinking`, and null values are rejected. |
...
| Model capability mode | Which reasoning request protocol Inx should use for this provider. | Keep **Auto-detect** unless the gateway is misdetected or the model docs require a specific reasoning format. |
...
| Context window | The provider-wide token budget Inx uses for automatic context cleanup. `0` disables automatic compaction. | Set it to the provider's model context limit; use a per-model override below when selected models differ. |
...
| Option | Effect |
| --- | --- |
| Auto-detect (recommended) | Inx chooses the request shape from model capability metadata and endpoint detection. |
...
| Option | Effect |
| --- | --- |
| Auto (provider default) | Does not write an explicit provider-level `thinking` override. Inx uses the provider/model default behavior. |
...
`extra_body` is merged into the chat JSON request body. Inx keeps core
...
`hookSpecificOutput.additionalContext`; Inx injects that text once into the
...
`.claude/settings.json` command hooks are also mapped to matching Inx hook
events.
...
row, press a new key combination, and Inx saves it for the desktop app.
...
| `Cmd+Z` on macOS, `Ctrl+Z` on Windows/Linux | Undoes the latest composer edit | Native typing stays in the WebView history; Inx-managed paste, cut, folded blocks, and structured tokens are restored as complete transactions. |
...
| Right-click with no active selection | Pastes clipboard text locally | In a local session with in-app mouse capture on, Inx reads text only and routes it through the normal bracketed-paste handling. Over SSH, use the terminal paste shortcut because the remote process cannot read the local clipboard; `/mouse` restores the terminal's native right-click menu. Right-click with an active selection still copies that selection. |
| `/mouse` | Toggles in-app mouse capture | Off hands the mouse back to your terminal, restoring its native click-drag selection and right-click context menu, at the cost of in-app drag-select, the transcript scrollbar, and wheel-scroll. Set `INX_DISABLE_MOUSE=1` to start every session with it off. |
...
| Your terminal's text-paste shortcut | Pastes text | Text stays on the terminal's bracketed-paste path (`Cmd+V` on macOS, commonly `Ctrl+Shift+V` on Linux, and the terminal's configured shortcut elsewhere). Inx consumes the resulting paste event and never probes for an image first. |
...
| `/theme [auto|light|dark|style]` | Shows or switches the CLI theme | Bare `/theme` lists background modes and named accent palettes. The choice is saved to the user config; `INX_THEME` and `INX_THEME_STYLE` can override it for one run. |
...
`inx` can grant Bash as an exact command or as a conservative command
...
`inx run --auto ...`, `-y`, or `--permission-mode auto` when unattended
...
Inx always removes saved provider and bot credential variables from tool
...
For coding-quality reports, run `inx doctor quality <branch-id-or-path>`
...
or Discussion. This differs from `inx doctor session`, whose support zip
...
| Agent | `/inx-guide` (built-in inline skill) or ask naturally; it prefers static doctor JSON before `--live` |

Exit code `0` allows warnings/info; `1` means at least one `error` (or a live
start failure); `2` is bad flags. This is separate from `inx doctor`
(providers/sandbox) and `inx plugin doctor <name>` (one package).

## Plugins (MCP)

Inx is an MCP client. A `[[plugins]]` entry's `type` selects the transport:
...
registry**, or use `inx mcp browse [query]` and
`inx mcp install <registry-name>`. Registry access is explicit and never
...
connect**, `/mcp add`, or ask Inx to install a package or URL. These
...
project's `inx.toml` or `.mcp.json` remain in that project and are trusted
...
same-name global installs; inside a project, `inx.toml` overrides
...
Inx deliberately trusts an installed server to describe those hints
...
loading older files and removed the next time Inx saves that MCP entry.
...
exposes. `make build` also produces `bin/inx-plugin-example` — a runnable
...
[Capability diagnostics](./CAPABILITY_DIAGNOSTICS.md)
(`inx doctor capabilities` or **Settings → Diagnostics**).
...
**Upgrading from `0.x`?** Your old `~/.inx/config.json` is still read for its
`mcpServers` (honouring `mcpDisabled`) as a lowest-priority source, so MCP servers
keep working — move them into `inx.toml`'s `[[plugins]]` or a `.mcp.json` when
convenient.

## Slash commands

In an interactive `inx` session, built-in commands (`/compact`, `/new`, `/clear`, `/rewind`,
...
`/explore`, `/test`, and `/inx-guide` also appear in the slash menu and via
`run_skill` (bodies load on demand; only the index line is cache-stable). Use
`/inx-guide` when you need config or capability troubleshooting; it points
at `inx doctor capabilities` (see
...
Markdown files under `.inx/commands/` (project) or `~/.inx/commands/`
...
one with `/<name> <task>`; Inx runs an isolated child loop and keeps only
...
changing the ordinary `inx run` task semantics:

```bash
inx subagent list
inx subagent create reviewer --description "Review changes" --prompt-file reviewer.md --tools read_file,grep,bash
inx subagent edit reviewer --effort high --model deepseek-pro
inx subagent try reviewer "review the current diff"   # always read-only
inx subagent run reviewer "review and fix the current diff"
inx subagent delete reviewer --yes
```
...
- **Standing instructions** come from hierarchical `INX.md`, `AGENTS.md`,
...
Inx automatically recalls a small set of relevant facts before each real
...
`.inx/autoresearch/<task-id>/` directory. Advanced users can force it with
...
`.inx/autoresearch/<task-id>/` directory. For new tasks, the default id
shape is `YYYYMMDD-HHMMSS-slug`, such as `20260618-224530-cache-audit`; Inx
...
broad requirement. Dynamic run state stays in `.inx/autoresearch/...`, not
in `INX.md`, `AGENTS.md`, project memory, tool schemas, or the cache-stable
...
Embed `@` references in a message and Inx resolves them before sending, as
...
`inx setup` manages providers, model lists, credentials, connection tests,
...
The planner sees loaded `INX.md` / `AGENTS.md` memory and a small read-only
...
Inx routes each turn deterministically without another classifier model:
...
Inx manages normal execution automatically: if an active todo produces no
...
`Shift+Tab` in the CLI. Inx first drafts a plan, then waits for approval
...
session, or `inx config reasoning-language auto|zh|en` in a shell/script.
Pass `--local`
