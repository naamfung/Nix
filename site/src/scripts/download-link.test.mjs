...
test("desktop updater deep link selects the desktop pane", () => {
  assert.equal(downloadPaneFromURL("https://inx.io/?download=desktop#start"), "desktop");
});

test("plain install links preserve the default pane", () => {
  assert.equal(downloadPaneFromURL("https://inx.io/#start"), "");
  assert.equal(downloadPaneFromURL("https://inx.io/?download=desktop"), "");
});
...
test("download tab navigation keeps the pane and removes legacy channels", () => {
  assert.equal(
    downloadURLForPane("https://inx.io/?download=desktop&channel=preview#start", "cli", "stable"),
    "https://inx.io/?download=cli#start",
  );
  assert.equal(
    downloadURLForPane("https://inx.io/?download=desktop&channel=preview#start", "npm", "preview"),
    "https://inx.io/?download=npm#start",
  );
  assert.equal(downloadURLForPane("https://inx.io/", "desktop", "preview"),
    "https://inx.io/?download=desktop#start");
  assert.equal(downloadURLForPane("https://inx.io/", "unknown", "stable"), "");
});
