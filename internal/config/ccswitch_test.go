...
func TestLoadCCSwitchLegacyConfigPrefersInxFlag(t *testing.T) {
	dir := t.TempDir()
		...
		"mcp": {
			"servers": {
				...
				"inx-off": {
					"name": "inx-off",
					"server": {"command": "node", "args": ["off.js"]},
					"apps": {"codex": true, "inx": false}
				},
				"inx-on": {
					"name": "inx-on",
					"server": {"command": "node", "args": ["on.js"]},
					"apps": {"codex": false, "inx": true}
				}
			}
		}
	}`
...
func TestLoadCCSwitchMCPDBPrefersInxColumn(t *testing.T) {
	...
	setup := `CREATE TABLE mcp_servers (
		...
		enabled_inx BOOLEAN NOT NULL DEFAULT 0
	);
	INSERT INTO mcp_servers VALUES ('codex-only', 'codex-only', '{"command":"node","args":["codex.js"]}', 1, 0);
	INSERT INTO mcp_servers VALUES ('inx-only', 'inx-only', '{"command":"node","args":["inx.js"]}', 0, 1);`
	...
	if len(got) != 1 || got[0].Name != "inx-only" {
		t.Fatalf("entries = %+v, want only explicit Inx enablement", got)
	}
}

func TestLoadCCSwitchMCPDBFallsBackToCodexWithoutInxColumn(t *testing.T) {
	if _, err := exec.LookPath("sqlite3"); err != nil {
		t.Skip("sqlite3 not available")
	}
	dbPath := filepath.Join(t.TempDir(), "cc-switch.db")
